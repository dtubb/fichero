#!/usr/bin/env python3
"""
Simple script to add IIIF import functionality
"""
...
# Add IIIF command after the structure command
structure_end = '''@library_app.command("structure")
...
iiiiif_command = '''@library_app.command("structure")
def structure(collection_id: str, depth: int = 3):
    """Preview collection structure"""
    asyncio.run(_structure_collection(collection_id, depth))
...
content = content.replace(structure_end, iiiif_command)

# Add IIIF methods before the last method

...
final class ArtifactStore: ObservableDomainStore {
    ...
    /// Delete the given artifacts, splicing only the SUCCEEDED ids out of
    /// `items` in place (#4824 — no wholesale `reload()`). The store — not
    /// the view — owns the endpoint (observable-data-layer rule). Best-effort
    /// batch: a failure on one id is logged, the rest still attempt, and a
    /// failed id's row is left untouched by construction (it's never added to

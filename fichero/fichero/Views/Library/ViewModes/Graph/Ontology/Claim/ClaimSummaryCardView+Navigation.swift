...
//
// Pure move: no renames, no signature changes — EXCEPT that `private` had to
// go. Swift's `private` is FILE-scoped, not type-scoped, so members that were
// private inside one file become invisible to the rest of the type the moment
// the type spans two files. These are internal now; the enclosing type is the
// only caller either way.
//
// This is the third build break from a file split this month. `private` on a
// member of a type that lives in more than one file is not a narrower access
// level — it is a compile error waiting for the next split.

...
extension AISettingsView {
    ...
    func loadModelsResettingSelection(
        ...
        Task { @MainActor in
            await Self.applyModelLoad(
                providerType: providerType,
                fetch: {
                    let configured = try await appState.providerService.listProviderModels(providerId: provider.id)
                    return Self.configuredModelInfos(from: configured, providerType: providerType)
                },
                models: models, selection: selection, loadError: loadError
            )
        }
    }

    /// The load itself, with the fetch injected so a test can drive both outcomes (#5052).
    @MainActor
    static func applyModelLoad(
        providerType: String,
        fetch: @MainActor () async throws -> [ModelInfo],
        models: Binding<[ModelInfo]>,
        ...
        do {
            ...
            // shape `AISettingsSelectionTests.aFailedFetchKeepsSelectionAndList`
            // and its siblings assert — rather than an unused pure function nothing actually applies.

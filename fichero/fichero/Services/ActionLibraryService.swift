...
/// sites pass `headers: .init()` and never hand-pass the library header.
@MainActor
...
class ActionLibraryService: ObservableObject {
    ...
    func loadActions() async {
        ...
        do {
            let response = try await client.api.listActionsApiActionsGet(headers: .init())
            switch response {
    ...
    func loadCategories() async {
        do {
            let response = try await client.api.listCategoriesApiActionsCategoriesGet(headers: .init())
            switch response {
            case .ok(let okResponse):
    ...
    func loadBuiltinActions() async -> [ActionItem] {
        do {
            let response = try await client.api.listBuiltinActionsApiActionsBuiltinGet(headers: .init())
            switch response {
            case .ok(let okResponse):
                return try decodeModels(from: try okResponse.body.json.items, as: ActionItem.self)
            case .unprocessableContent, .undocumented:
    ...
    func loadCustomActions() async -> [ActionItem] {
        do {
            let response = try await client.api.listCustomActionsApiActionsCustomGet(headers: .init())
            switch response {
            case .ok(let okResponse):
                return try decodeModels(from: try okResponse.body.json.items, as: ActionItem.self)
            case .unprocessableContent, .undocumented:

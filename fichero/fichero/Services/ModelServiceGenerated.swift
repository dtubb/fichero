...
private let logger = Logger(subsystem: "app.fichero.fichero", category: "ModelServiceGenerated")
...
class ModelServiceGenerated {
    ...
    func searchHuggingFaceModels(
        ...
        case .unprocessableContent(let error):
            ...
            throw ModelServiceGeneratedError.serverError(detail?.detail?.description ?? "Validation error")
        case .undocumented(let statusCode, _):
            throw ModelServiceGeneratedError.unexpectedResponse(statusCode)
        }
    }
...
enum ModelServiceGeneratedError: Error, LocalizedError {

...
// MARK: - Stage Variant Metadata Parsing Tests (#1174)

struct StageVariantStateTests {

    @Test("Parses stage variants and operation labels from document metadata")
    func parsesStageVariants() {
        let doc = Document(
            id: "doc-stage",
            docType: .page,
            fileType: .pdf,
        ...
        let state = StageVariantState.from(document: doc)
        #expect(state.stages[.stageA]?.count == 1)
        #expect(state.stages[.stageA]?.first?.isCanonical == true)
        #expect(state.stages[.stageA]?.first?.operations.map(\.label) == ["deskew", "enhance"])
        #expect(state.stages[.stageB]?.first?.operationCount == 3)
    }

    @Test("Ignores malformed stage variant entries")
    func ignoresMalformedEntries() {
        ...
        let state = StageVariantState.from(document: doc)
        #expect(state.stages[.stageA]?.count == 1)
        #expect(state.stages[.stageA]?.first?.id == "ok")
    }
}

...
# =============================================================================
# Feature Flag: Native Menu/Toolbar System
# =============================================================================
# Set to True to use the new native NSMenu/NSToolbar implementation.
# Set to False to use the existing FicheroCommand system.
# Once testing is complete, this flag and the legacy code can be removed.

USE_NATIVE_MENU_TOOLBAR = True  # Enable new native menu/toolbar system
...
class MainWindow:
    ...
    def __init__(self, app):
        ...
        # Use feature flag to choose between native and legacy systems
        if not self.is_mobile:
            if USE_NATIVE_MENU_TOOLBAR:
                self._setup_native_menu_toolbar()
            else:
    ...
    def _create_desktop_layout(self):
        ...
        try:
            ...
            if self.navigation_controller and self.navigation_controller.layout_manager:
                ...
                # Wire up main_window reference for dialogs (export functionality)
                if hasattr(adjust_view, 'set_main_window'):
                    adjust_view.set_main_window(self)
                    logger.info("✅ AdjustView wired to main_window for dialogs")
    ...
    # =========================================================================
    # Native Menu/Toolbar System (Feature Flag: USE_NATIVE_MENU_TOOLBAR)
    # =========================================================================

    def _setup_native_menu_toolbar(self):
        """
        Set up native macOS menu bar and toolbar.

        This is the new system that replaces FicheroCommand/MacToolbarManager.
        Uses simple declarative definitions in menu.py and toolbar.py.

        Menu: Built immediately, then rebuilt in on_running after Toga sets its menu.
        Toolbar: Created here (with delegate set), attached in on_running.
        """
        logger.info("🔧 _setup_native_menu_toolbar() called")
        try:
            from fichero.windows.main.menu import AppMenu
            logger.info("🔧 AppMenu imported successfully")

            # Create and build native menu bar
            self._native_menu = AppMenu(handler=self)
            logger.info("🔧 AppMenu instance created")
            self._native_menu.build()
            logger.info("🔧 Native menu bar built")

            # Create native toolbar (uses class-level attributes like MacToolbarManager)
            from fichero.windows.main.toolbar import AppToolbar
            self._native_toolbar = AppToolbar(
                handler=self,
                on_search=self._on_toolbar_search
            )
            logger.info("🔧 Native toolbar created (will attach in on_running)")

        except Exception as e:
            logger.error(f"Failed to set up native menu/toolbar: {e}")
            import traceback
            logger.error(traceback.format_exc())

    def _on_toolbar_search(self, query: str):
        """Handle search from native toolbar."""
        logger.info(f"Toolbar search: {query}")
        # TODO: Implement search functionality
        # This could filter the collection view or search across libraries

    def attach_native_toolbar(self):
        """
        Attach the native toolbar to the window.

        Called from on_running callback after window is fully set up.
        """
        if not hasattr(self, '_native_toolbar') or not self._native_toolbar:
            logger.warning("No native toolbar to attach")
            return

        try:
            native_window = self.window._impl.native
            native_window.setToolbar(self._native_toolbar.native)
            logger.info("🔧 Native toolbar attached to window")
        except Exception as e:
            logger.error(f"Failed to attach native toolbar: {e}")
            import traceback
            logger.error(traceback.format_exc())

    # =========================================================================
    # Native Menu/Toolbar Handler Methods
    # =========================================================================
    # These methods are called by the native menu/toolbar system.
    # They delegate to existing methods or view methods.
    # Handler names must match the handler strings in menu.py and toolbar.py.
    ...
    def new_collection(self):
        ...
        if self.cached_library_view:
            self.cached_library_view._on_new_collection(None)

    def new_from_folder(self):
        """File > New from Folder..."""
        if self.cached_library_view:
            self.cached_library_view._on_new_collection_from_folder(None)

    def import_file(self):
        """File > Import File..."""
        if self.cached_collection_view:
            self.cached_collection_view._on_import_file(None)

    def import_folder(self):
        """File > Import Folder..."""
        if self.cached_collection_view:
            self.cached_collection_view._on_import_folder(None)
    ...
    # --- Edit Menu Handlers ---
    # These delegate to the first responder in the native responder chain

    def undo(self):
        """Edit > Undo"""
        pass  # Handled by responder chain
    ...
    def toggle_library(self):
        """View > Library (Cmd+Opt+1)"""
        self._toggle_library_pane(None)

    def toggle_collection(self):
        """View > Collection (Cmd+Opt+2)"""
        self._toggle_collection_pane(None)

    def toggle_preview_image(self):
        """View > Preview Image (Cmd+Opt+3)"""
        self._toggle_preview_image_pane(None)

    def toggle_preview_metadata(self):
        """View > Preview Metadata (Cmd+Opt+4)"""
        self._toggle_preview_metadata_pane(None)

    def toggle_inspector(self):
        """View > Info (Cmd+Opt+5)"""
        self._toggle_inspector_pane(None)

    def cycle_ratios(self):
        """View > Cycle Preview Ratios (Cmd+R)"""
        self._cycle_preview_ratios(None)

    def toggle_path_bar(self):
        """View > Path Bar"""
        pass  # TODO: Implement path bar toggle

    def toggle_status_bar(self):
        """View > Status Bar"""
        self._toggle_status_bar(None)

    # --- Tools Menu Handlers ---

    def process(self):
        """Tools > Process (Cmd+Return)"""
        if self.cached_collection_view:
            self.cached_collection_view._on_process_selected(None)

    def process_crop(self):
        """Tools > Crop"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('crop', None)

    def process_rotate(self):
        """Tools > Rotate"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('rotate', None)

    def process_enhance(self):
        """Tools > Enhance"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('enhance', None)

    def process_transcribe(self):
        """Tools > Transcribe"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('transcribe', None)

    def process_describe(self):
        """Tools > Describe"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('describe', None)

    def process_llm(self):
        """Tools > LLM Catalogue"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('llm_process', None)

    def process_convert_word(self):
        """Tools > Convert to Word"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('convert_to_word', None)

    # --- Window Menu Handlers ---

    def minimize(self):
        """Window > Minimize"""
        if self.window and hasattr(self.window, '_impl'):
            self.window._impl.native.miniaturize_(None)

    def zoom_window(self):
        """Window > Zoom"""
        if self.window and hasattr(self.window, '_impl'):
            self.window._impl.native.zoom_(None)

    def open_settings(self):
        """Window > Settings..."""
        if self.cached_library_view:
            self.cached_library_view._on_settings(None)
    ...
    def about(self):
        """App Menu > About Fichero"""
        if self.cached_library_view:
            self.cached_library_view._on_about(None)

    def hide_app(self):
        """App Menu > Hide Fichero"""
    ...
    def quit_app(self):
        """App Menu > Quit Fichero"""
        self.app.exit()

    # --- Toolbar Handlers ---

    def import_menu(self):
        """Toolbar: Import button (shows import dialog)"""
        if self.cached_collection_view:
            self.cached_collection_view._on_import_file(None)

    def _handle_search(self, query: str):
        """Toolbar: Search field text changed"""
        if self.cached_library_view:
            self.cached_library_view._on_search(query)

    # --- Additional File Menu Handlers ---

    def new_collection_from_folder(self):
        """File > New External Collection..."""
        if self.cached_library_view:
            self.cached_library_view._on_new_collection_from_folder(None)
    ...
    def delete_collection(self):
        ...
        if self.cached_library_view:
            self.cached_library_view._on_delete_collection(None)

    def close_all(self):
        """File > Close All"""
        # Close all windows except main
        pass  # TODO: Implement close all windows

    def import_url(self):
        """File > Import > URL..."""
        if self.cached_collection_view:
            self.cached_collection_view._on_import_url(None)

    def link_file(self):
        """File > Import > Link File..."""
        if self.cached_collection_view:
            self.cached_collection_view._on_link_file(None)

    def link_folder(self):
        """File > Import > Link Folder..."""
        if self.cached_collection_view:
            self.cached_collection_view._on_link_folder(None)
    ...
    def zoom_actual_size(self):
        """View > Actual Size"""
        if self.cached_collection_view:
            self.cached_collection_view._zoom_actual_size(None)

    def zoom_in(self):
        """View > Zoom In"""
        if self.cached_collection_view:
            self.cached_collection_view._zoom_in(None)

    def zoom_out(self):
        """View > Zoom Out"""
        if self.cached_collection_view:
            self.cached_collection_view._zoom_out(None)

    def zoom_to_fit(self):
        """View > Zoom to Fit"""
        if self.cached_collection_view:
            self.cached_collection_view._zoom_to_fit(None)

    def zoom_to_selection(self):
        """View > Zoom to Selection"""
        if self.cached_collection_view:
            self.cached_collection_view._zoom_to_selection(None)

    def magnifier_zoom_in(self):
        """View > Magnifier Zoom In"""
        if self.cached_collection_view:
            self.cached_collection_view._magnifier_zoom_in(None)

    def magnifier_zoom_out(self):
        """View > Magnifier Zoom Out"""
        if self.cached_collection_view:
            self.cached_collection_view._magnifier_zoom_out(None)

    def toggle_magnifier(self):
        """View > Show Magnifier"""
        if self.cached_collection_view:
            self.cached_collection_view._toggle_magnifier(None)

    def toggle_markup_toolbar(self):
        """View > Show Markup Toolbar"""
        pass  # TODO: Implement markup toolbar toggle
    ...
    def go_next(self):
        """Go > Next Item"""
        if self.cached_collection_view:
            self.cached_collection_view._go_next(None)

    def go_previous(self):
        """Go > Previous Item"""
        if self.cached_collection_view:
            self.cached_collection_view._go_previous(None)

    # --- Additional Tools Menu Handlers ---

    def process_segment(self):
        """Tools > Segment Images"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('segment', None)

    def process_recombine(self):
        """Tools > Recombine Segments"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('recombine_segments', None)

    def process_split(self):
        """Tools > Split Images"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('split', None)

    def process_prepare(self):
        """Tools > Prepare Images"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('prepare', None)

    def process_remove_background(self):
        """Tools > Remove Background"""
        if self.cached_collection_view:
            self.cached_collection_view._on_tool_selected('remove_background', None)

    # --- Additional Window Menu Handlers ---
    ...
    def show_activity(self):
        """Window > Activity"""
        pass  # TODO: Show activity window

    def show_output(self):
        """Window > Output"""
        pass  # TODO: Show output window

    def show_processing(self):
        """Window > Processing"""
        pass  # TODO: Show processing window

    def show_plans(self):
        """App Menu > Plans..."""
        if self.cached_library_view:
            self.cached_library_view._on_plans(None)

    def show_prompts(self):
        """App Menu > Prompts..."""
        if self.cached_library_view:
            self.cached_library_view._on_prompts(None)

    # --- Additional Help Menu Handlers ---

    def visit_homepage(self):
        """Help > Visit Homepage"""
        import webbrowser
        webbrowser.open("https://github.com/tubbca/fichero")
    ...
    def search(self):
        """Toolbar: Search (placeholder - handled by search field callback)"""
        pass  # Search is handled by _handle_search callback

    # =========================================================================
    # Legacy Menu/Toolbar System (when USE_NATIVE_MENU_TOOLBAR = False)
    # =========================================================================
    ...
    def _on_show_collection(self, event):
        ...
        try:
            ...
            # Update AdjustView with current collection for export functionality
            adjust_view = self.views.get('adjust')
            if adjust_view and hasattr(adjust_view, 'set_collection'):
                adjust_view.set_collection(collection_id, collection_name)

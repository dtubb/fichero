...
class CollectionView(BaseView):
    ...
    def __init__(self, app, collection_name: str = "", is_mobile: bool = False, collection_id: Optional[str] = None):
        ...
        # Last selected item (preserved for processing even when widget selection clears)
        self._last_selected_item: Optional[Dict[str, Any]] = None
        ...
        # Processing state guard to prevent rapid double-clicks
    ...
    def define_commands(self):
        ...
        try:
            ...
            go_group = toga.Group("Go", order=40)  # After View (30), before Tools (50)
            ...
            self.commands = {
                ...
                'go_previous': FicheroCommand(
                    id='collection.go_previous',
                    label=_("Previous Item"),
                    action=self._on_go_previous,
                    shortcut=toga.Key.UP,  # Up arrow
                    description=_("Select previous item in list"),
                    group=go_group,
                    section=1,
                    order=0,
                    show_in_menu=True,
                ...
                'go_next': FicheroCommand(
                    id='collection.go_next',
                    label=_("Next Item"),
                    action=self._on_go_next,
                    shortcut=toga.Key.DOWN,  # Down arrow
                    description=_("Select next item in list"),
                    group=go_group,
                    section=1,
                    order=1,
                    show_in_menu=True,
    ...
    def _on_go_previous(self, widget=None):
        """Handle Go > Previous Item - select previous item in list"""
        try:
            logger.info("Go menu: Previous item requested")
            if hasattr(self, 'items_list') and self.items_list:
                result = self.items_list.select_previous()
                if result:
                    logger.debug("Selected previous item")
                else:
                    logger.debug("Already at first item or no items")
            else:
                logger.warning("No items_list available - cannot select previous")
        except Exception as e:
            logger.error(f"Failed to select previous item: {e}")

    def _on_go_next(self, widget=None):
        """Handle Go > Next Item - select next item in list"""
        try:
            logger.info("Go menu: Next item requested")
            if hasattr(self, 'items_list') and self.items_list:
                result = self.items_list.select_next()
                if result:
                    logger.debug("Selected next item")
                else:
                    logger.debug("Already at last item or no items")
            else:
                logger.warning("No items_list available - cannot select next")
        except Exception as e:
            logger.error(f"Failed to select next item: {e}")
    ...
    def _create_collection_items_list(self, items: List[Dict[str, Any]]):
        ...
        try:
            ...
            self.items_list = ListWidget(
                ...
                multiple_select=False,  # Single selection only
                style=Pack(
                    flex=1,
                ...
                initial_path=self.current_path or ""  # Start at current folder path
            )
            ...
            if not items:
                ...
                logger.debug(f"Created HTML list with {len(items)} items at path '{self.current_path}'")
    ...
    def _on_item_selected(self, widget_or_item):
        ...
        try:
            ...
            if selected_item_ids:
                ...
                # Store last selected item for processing (survives focus loss)
                self._last_selected_item = first_item_data.copy() if first_item_data else None
                logger.debug(f"📌 Stored _last_selected_item: {self._last_selected_item.get('id') if self._last_selected_item else None}")
                ...
                if first_item_data.get('is_folder', False) or first_item_data.get('type') == 'folder':
                    # Folder selected - navigate into it (Finder-like behavior)
                    logger.info(f"📂 Folder selected: {first_item_data.get('name', first_item_data.get('title'))} - navigating into it")
                    self._handle_item_navigation(first_item_data)
                else:
    ...
    async def _on_process_requested(self, widget):
        """Process the current sidebar selection.

        Respects folder navigation - only processes what's currently visible:
        - If at root of collection: processes all items in collection
        - If inside a folder: processes only items within that folder (recursively)
        - If a specific item is selected in collection view: processes just that item
        """
        import time

        try:
            # Guard against rapid double-clicks (within 1 second)
            current_time = time.time()
            if current_time - self._last_process_time < 1.0:
                logger.warning(f"⚠️ Process requested too quickly ({current_time - self._last_process_time:.2f}s since last), ignoring")
                return

            self._last_process_time = current_time

            # Use view's collection_id - this is what's selected in the sidebar
            if not self.collection_id:
                logger.error("No collection selected - please select a collection in the sidebar")
                return

            # Log the current context
            logger.info(f"Process clicked - collection: {self.collection_id}, current_path: '{self.current_path}'")

            # Check if there's a specific item selected in the collection view
            selected_item = self._get_selected_collection_item()

            if selected_item:
                # Process just the selected item (or its children if it's a folder)
                logger.info(f"Processing selected item: {selected_item.get('id')} ({selected_item.get('title', 'Unknown')})")
                ...
                # No item selected - process based on current_path (folder context)
                logger.info(f"No item selected - processing folder/collection context")
                await self._show_process_dialog(self.collection_id, None, None)
    ...
    def _get_selected_collection_item(self) -> Optional[Dict[str, Any]]:
        """Get the currently selected item in the collection view (if any).

        Uses _last_selected_item as primary source since ListWidget selection
        gets cleared when focus moves to toolbar/menu.

        Returns:
            Dict with item data if an item is selected, None otherwise.
        """
        try:
            # Primary: Use _last_selected_item which persists across focus changes
            if hasattr(self, '_last_selected_item') and self._last_selected_item:
                logger.info(f"🔍 Using _last_selected_item: id={self._last_selected_item.get('id')}, title={self._last_selected_item.get('title', 'N/A')}")
                return self._last_selected_item

            # Fallback: Try SelectionManager
            if hasattr(self.app, 'selection_manager') and self.app.selection_manager:
                sm = self.app.selection_manager
                item_ids = sm.get_selection('collection')
                logger.debug(f"🔍 SelectionManager.get_selection('collection') = {item_ids}")

                if item_ids and len(item_ids) > 0:
                    metadata_list = sm.get_selection_metadata('collection')
                    item_id = item_ids[0]
            ...
            # Fallback: Try ListWidget directly (may be None if focus moved)
            if hasattr(self, 'items_list') and self.items_list:
                selection = self.items_list.get_selection()
                logger.debug(f"🔍 ListWidget selection: {selection}, type={type(selection)}")

                if selection:
                    if hasattr(selection, '__class__') and 'Row' in selection.__class__.__name__:
                        if hasattr(selection, '_item_data'):
                            return selection._item_data
                        item_id = getattr(selection, 'id', None) or getattr(selection, '_id', None)
                        if item_id:
                            return {
                                'id': item_id,
                                'title': getattr(selection, 'text', '') or getattr(selection, 'title', ''),
                                'type': getattr(selection, 'type', 'file'),
                                'is_folder': getattr(selection, 'is_folder', False),
                            }

                    if isinstance(selection, dict):
                        if '_item_data' in selection:
                            return selection['_item_data']
                        if 'id' in selection:
                            return selection
                        return selection

            logger.debug("_get_selected_collection_item: No selection found")
            return None

        except Exception as e:
            logger.error(f"Failed to get selected collection item: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    async def _show_process_dialog(self, collection_id: str, selected_item_id: Optional[str] = None, selected_item_name: Optional[str] = None):
        """Process items based on current selection context.

        Priority:
        1. If selected_item_id is provided, process that item (and children if folder)
        2. If current_path is set, process items within that folder path
        3. Otherwise process all items at root level of collection
        """
        try:
            ...
            # Get items to process based on context
            items_to_process = await self._get_items_to_process(
                collection_id, selected_item_id
            )

            if items_to_process:
                # We have items to process - use DirectorIntegrationService
                await self._process_via_items(collection_id, collection, items_to_process, selected_item_id, selected_item_name)
            else:
                # No items found
                if selected_item_id:
                    logger.error(f"No processable items found for selected item: {selected_item_id}")
                elif self.current_path:
                    logger.error(f"No items found in folder: {self.current_path}")
                else:
                    # Try folder-based processing as fallback
    ...
    async def _get_items_to_process(self, collection_id: str, selected_item_id: Optional[str] = None) -> List:
        """Determine which items to process based on current context.

        Args:
            collection_id: The collection ID
            selected_item_id: Specific item selected (optional)

        Returns:
            List of CollectionItem objects to process
        """
        try:
            # Case 1: Specific item selected
            if selected_item_id:
                # Get the item first to check if it's a folder
                item = await self.app.library_manager.get_item(selected_item_id)
                if not item:
                    logger.warning(f"Selected item {selected_item_id} not found")
                    return []

                if item.type == "folder":
                    # It's a folder - get all items recursively within it
                    logger.info(f"Selected item is a folder - getting recursive contents")
                    folder_items = await self.app.library_manager.get_folder_items_recursive(
                        selected_item_id, include_folders=False
                    )
                    logger.info(f"Found {len(folder_items)} items in folder {item.name}")
                    return folder_items
                else:
                    # It's a file - process just this item
                    logger.info(f"Selected item is a file - processing single item")
                    return [item]

            # Case 2: Inside a folder (current_path is set)
            if self.current_path:
                logger.info(f"Processing items within current path: '{self.current_path}'")
                # Get all items in collection and filter by path
                all_items = await self.app.library_manager.get_collection_items(collection_id)

                # Filter to items within current_path (and subfolders)
                items_in_path = []
                for item in all_items:
                    # Skip folders themselves - we want processable items
                    if item.type == "folder":
                        continue

                    # Check item's path from metadata
                    item_path = item.metadata.get('relative_path', '')

                    # Item is in path if:
                    # - item_path equals current_path
                    # - OR item_path starts with current_path + '/'
                    if item_path == self.current_path:
                        items_in_path.append(item)
                    elif item_path.startswith(self.current_path + '/'):
                        items_in_path.append(item)

                logger.info(f"Found {len(items_in_path)} items in path '{self.current_path}'")
                return items_in_path

            # Case 3: At root level - process all non-folder items in collection
            logger.info(f"At root level - processing all items in collection")
            all_items = await self.app.library_manager.get_collection_items(collection_id)

            # Filter to non-folder items only
            processable_items = [item for item in all_items if item.type != "folder"]
            logger.info(f"Found {len(processable_items)} processable items in collection (excluding folders)")
            return processable_items

        except Exception as e:
            logger.error(f"Failed to determine items to process: {e}")
            import traceback
            traceback.print_exc()
            return []

    async def _process_via_items(self, collection_id: str, collection, items_to_process, selected_item_id: Optional[str] = None, selected_item_name: Optional[str] = None):
        """Process using DirectorIntegrationService (items from database)

        Args:
            collection_id: Collection ID
            collection: Collection object
            items_to_process: Pre-filtered list of items to process (already respects folder context)
            selected_item_id: Specific item ID if one was selected
            selected_item_name: Name of selected item for logging
        """
        # Extract item IDs from the pre-filtered list
        item_ids = [item.id for item in items_to_process]

        # Build descriptive scope text for logging
        if selected_item_id and selected_item_name:
            scope_text = f"Selected: {selected_item_name} ({len(item_ids)} items)"
        elif self.current_path:
            scope_text = f"Folder '{self.current_path}': {len(item_ids)} items"
        else:
            scope_text = f"Collection '{collection.name}': {len(item_ids)} items"

        logger.info(f"📦 Processing scope: {scope_text}")
            ...
            try:
                ...
                if navigation_state.context == NavigationContext.COLLECTION:
                    ...
                    if collection_changed or path_changed:
                        ...
                        # Clear last selected item when navigating to different collection/folder
                        self._last_selected_item = None
                        logger.debug("🔄 Cleared _last_selected_item on navigation change")

...
class SidebarDataModel:
    ...
    def to_source_list_items(self) -> List["SourceListItem"]:
        """Convert model to SourceListItem tree for SourceList widget.

        Returns:
            List of SourceListItem objects with hierarchical structure
        """
        ...
        for section in sorted_sections:
            ...
            # Build collection children
            collection_children = []
            for collection in sorted_collections:
                # Get icon - use SF Symbol based on type
                icon = self._get_sf_symbol_for_type(collection.type, collection.metadata)

                # Build folder children if any
                folder_children = []
                collection_folders = self.folders.get(collection.id, [])
                if collection_folders:
                    sorted_folders = sorted(collection_folders, key=lambda f: (f.sort_order, f.name.lower()))
                    for folder in sorted_folders:
                        folder_item = SourceListItem(
                            id=folder.id,
                            text=folder.name,
                            icon="folder.fill",
                            is_header=False,
                        ...
                        folder_children.append(folder_item)

                # Create collection item
                is_inbox = collection.metadata.get("is_inbox", False)
                item_count = collection.item_count or 0
                badge = str(item_count) if item_count > 0 else None
                collection_item = SourceListItem(
                    id=collection.id,
                    text=collection.name,
                    icon=icon,
                    badge=badge,
                    is_header=False,
                    accepts_drops=not is_inbox,
                ...
                collection_children.append(collection_item)

            # Create section header
            section_item = SourceListItem(
                ...
                data={"type": "section", "section_id": section.id},
            )
            items.append(section_item)

        return items

    def _get_sf_symbol_for_type(self, collection_type: str, metadata: Dict[str, Any]) -> str:
        """Get SF Symbol name for collection type."""
        ...
        symbols = {
            ...
            "external": "link",
            "url": "globe",
            "hybrid": "arrow.triangle.2.circlepath",
        }
        return symbols.get(collection_type, "folder.fill")

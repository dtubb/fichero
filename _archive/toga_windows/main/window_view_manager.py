    ...
    def _create_desktop_window(self, window_type: WindowType, **kwargs):
        ...
        try:
            ...
            if window_type == WindowType.SETTINGS:
                from fichero.app import SettingsWindow
                window = SettingsWindow(self.app)

            elif window_type == WindowType.ABOUT:
                from fichero.app import AboutWindow
                window = AboutWindow(self.app)

            elif window_type == WindowType.ACTIVITY_MONITOR:
                from fichero.app import ActivityMonitorWindow
                window = ActivityMonitorWindow(self.app)

            # Plans, Prompts, Processing, Preview windows removed
            elif window_type in (WindowType.PLANS, WindowType.PROMPTS,
                                WindowType.PROCESSING, WindowType.PREVIEW,
                                WindowType.ADD_DIALOG):
                logger.warning(f"Window type {window_type.value} has been removed")

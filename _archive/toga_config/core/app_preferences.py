...
def get_app_data_directory(app=None) -> Path:
    """
    Get the app data directory.

    Args:
        app: Toga app instance (optional, uses app.paths.data if available)

    Returns:
        Path to app data directory
    """
    # Use Toga app paths if available
    if app and hasattr(app, 'paths') and hasattr(app.paths, 'data'):
        return app.paths.data

    # Otherwise use storage settings (canonical source)

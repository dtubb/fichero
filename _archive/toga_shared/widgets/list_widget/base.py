...
class ListWidget:
    ...
    def __init__(
        ...
        initial_path: str = "",  # Starting path for navigable lists (empty = root)
    ):
        """
        ...
        Args:
            ...
            initial_path: Starting path for navigable lists (empty string = root)
        """
        logger.info(f"🎬 ListWidget.__init__: renderer={renderer}, navigable={navigable}, data_count={len(data) if data else 0}, initial_path='{initial_path}'")
        if data:
        ...
        self.current_path = initial_path  # Use initial_path, empty string = root level
    ...
    def get_selected_indices(self) -> List[int]:
        ...
        Returns:
            List of selected indices (empty list if nothing selected)
        """
        # For CardRenderer, use its selected_cards set
        if hasattr(self.renderer, 'selected_cards'):
            return sorted(list(self.renderer.selected_cards))

        # For native widgets, find index of selection in source
        selection = self.get_selection()
        if not selection or not self._source:
            return []
        ...
        if isinstance(selection, list):
            for sel in selection:
                try:
                    idx = list(self._source).index(sel)
                    indices.append(idx)
                except (ValueError, IndexError):
                    pass
        else:
            try:
                idx = list(self._source).index(selection)
                indices.append(idx)
            except (ValueError, IndexError):
                pass
    ...
    def select_by_index(self, index: int) -> bool:
        ...
        if not self._data or index < 0 or index >= len(self._data):
            logger.debug(f"select_by_index: invalid index {index}, data length={len(self._data) if self._data else 0}")
            return False

        # For CardRenderer, use its _handle_card_select method
        if hasattr(self.renderer, '_handle_card_select'):
            self.renderer._handle_card_select(index)
            return True

        # For native widgets with source
        if self._source and index < len(self._source):
            try:
                row = list(self._source)[index]
                if isinstance(self.widget, (toga.Table, toga.Tree, toga.DetailedList)):
                    self.widget.selection = row
                    return True
            except Exception as e:
                logger.warning(f"Failed to select by index: {e}")
    ...
    def select_next(self) -> bool:
        ...
        Returns:
            True if selection changed, False if at end or no data
        """
        if not self._data:
            return False
        ...
        # Get the last selected index and move to next
        current_index = indices[-1]
        next_index = current_index + 1

        if next_index >= len(self._data):
            logger.debug("select_next: already at last item")
            return False

        return self.select_by_index(next_index)
    ...
    def select_previous(self) -> bool:
        ...
        Returns:
            True if selection changed, False if at beginning or no data
        """
        if not self._data:
            return False
        ...
        if not indices:
            ...
            return self.select_by_index(len(self._data) - 1)

        # Get the first selected index and move to previous
        current_index = indices[0]
        prev_index = current_index - 1

        if prev_index < 0:
            logger.debug("select_previous: already at first item")
            return False

        return self.select_by_index(prev_index)

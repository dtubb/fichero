...
class NSOutlineViewSidebar(Renderer):
    ...
    def set_rename_callback(self, callback: callable):
        """
        Set callback for inline rename completion.

        The callback receives:
            item_id: str - The ID of the item being renamed (collection ID or text)
            new_name: str - The new name entered by the user

        Example:
            def on_rename(item_id, new_name):
                library_manager.update_collection(item_id, {'name': new_name})
                refresh_sidebar()

            sidebar.set_rename_callback(on_rename)
        """
        self._on_rename_callback = callback

    def start_rename_selected(self):
        """
        Start inline rename on the currently selected item.

        Called from File menu or external code to trigger Finder-like
        inline editing on the selected sidebar item.
        """
        if self._toga_sidebar and hasattr(self._toga_sidebar, 'start_rename_selected'):
            self._toga_sidebar.start_rename_selected()

...
class NavigationController:
    ...
    def _create_inspector_view(self, params: dict = None):
        """Show inspector - now integrated as Info pane in main window."""
        try:
            # Inspector is now the Info pane in the main window
            # Toggle it via the main window's toggle_inspector method
            if hasattr(self.app, 'main_window') and self.app.main_window:
                main_window = self.app.main_window
                if hasattr(main_window, 'toggle_inspector'):
                    main_window.toggle_inspector()
            return None
        except Exception as e:

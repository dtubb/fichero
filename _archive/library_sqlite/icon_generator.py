...
class IconGenerator:
    ...
    def _get_default_icon(self, item_type: str, file_ext: str = "") -> Optional['toga.Image']:
        ...
        try:
            ...
            # Get the base resource path for fichero
            # Try to find resources relative to this module
            module_path = Path(__file__).parent.parent  # fichero/library -> fichero
            resources_base = module_path / "resources"

            # Map types to icon files (relative to resources/)
            icon_map = {
                'folder': 'icons/toolbar/folder@10x.png',
                'url': 'icons/toolbar/link.png',
                'file': 'icons/toolbar/document.png',
                'camera': 'icons/toolbar/camera.png',
                'audio': 'icons/toolbar/audio.png',
            }

            # Extension-specific icons
            ext_icon_map = {
                '.pdf': 'icons/toolbar/document.png',
                '.doc': 'icons/toolbar/document.png',
                '.docx': 'icons/toolbar/document.png',
                '.txt': 'icons/toolbar/document.png',
            }

            # Try extension-specific first
            if file_ext and file_ext.lower() in ext_icon_map:
                icon_path = resources_base / ext_icon_map[file_ext.lower()]
                if icon_path.exists():
                    return toga.Image(icon_path)

            # Fall back to type-based icon
            if item_type in icon_map:
                icon_path = resources_base / icon_map[item_type]
                if icon_path.exists():
                    return toga.Image(icon_path)

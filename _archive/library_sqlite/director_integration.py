...
class DirectorIntegrationService:
    ...
    async def process_items(self, collection_id: str, item_ids: List[str],
        ...
        logger.info(f"=== PROCESSING ITEMS DEBUG ===")
        logger.info(f"Total item_ids received: {len(item_ids)}")
        ...
        for item_id in item_ids:
            ...
            logger.info(f"  Item: id={item_id[:8]}..., name='{item.name}', type={item.type}, path={input_path}")
        ...
        logger.info(f"Grouped: {len(folder_items)} folders, {len(file_items)} files")
                ...
                for parent_key, group_files in files_by_parent.items():
                    if len(group_files) == 1:
                        ...
                        try:
                            ...
                            # BUG FIX: Use the PARENT FOLDER's item_id for routing, not the first file's ID
                            # parent_key is the actual folder item_id that owns these files
                            # This is critical for item_map building in _process_single_folder
                            folder_item_id = parent_key if parent_key != "_root_" else group_files[0][0]
                            task_id_list = await self._process_single_folder(
                                folder_item_id, temp_dir, catalogue_output_path,
        ...
        try:
            if len(parent_dirs) == 1:
                ...
                task_id = self.director.processing_coordinator.process_folders(
                    ...
                    library_item_map=batch_item_map
                )
            else:
                ...
                for parent_dir in parent_dirs:
                    # Build item_map for files in this specific folder
                    folder_item_map = {k: v for k, v in batch_item_map.items()
                                      if any(item[2].parent == parent_dir for item in file_items if item[2].name == k)}
                    task_id = self.director.processing_coordinator.process_folders(
                        folders=[parent_dir],
                        plan_name=plan_name,
                        workflow_name=workflow_name,
                        ...
                        library_collection_id=collection_id,
                        library_item_map=folder_item_map
                    )
    ...
    async def _process_single_file(self, item_id: str, input_path: Path,
        ...
        try:
            ...
            # Build item_map for single file: filename -> item_id
            single_file_item_map = {input_path.name: item_id}
            task_id = self.director.processing_coordinator.process_folders(
                folders=[{
                ...
                library_collection_id=collection_id,
                library_item_map=single_file_item_map
            )
    ...
    async def _process_single_folder(self, item_id: str, input_path: Path,
        ...
        # Include library IDs for proper metadata routing in workflow_manifest.json
        try:
            task_id = self.director.processing_coordinator.process_folders(
                folders=[{
                    'output_folder': output_path,
        ...
        plan_expectations = {
            ...
            "Transcribir y Catalogar": {
                "Catalogue": {
                    ...
                    "transcribe_qwen_ocr": ["transcription"],
    ...
    def _normalize_filename(filename: str) -> str:
        ...
        Handles Unicode normalization (NFC vs NFD forms), which is critical
        for filenames containing accented characters (e.g., "Peña" vs "Peña").

        macOS HFS+ uses NFD (decomposed), while most other systems use NFC (composed).
        We normalize to NFC for consistent comparisons.
    ...
    def _normalized_filename_lookup(self, filename: str, lookup_dict: Dict[str, str]) -> Optional[str]:
        """Look up a filename in a dict with Unicode normalization.

        Tries exact match first, then normalized matching.

        Args:
            filename: Filename to look up
            lookup_dict: Dictionary mapping filenames to values

        Returns:
            Value from dict if found, None otherwise
        """
        # Try exact match first (most common case)
        if filename in lookup_dict:
            return lookup_dict[filename]

        # Try normalized comparison
        normalized_key = self._normalize_filename(filename)
        for dict_key, dict_value in lookup_dict.items():
            if self._normalize_filename(dict_key) == normalized_key:
                logger.debug(f"[UNICODE] Matched '{filename}' to '{dict_key}' via normalization")
                return dict_value

        return None

    def _resolve_output_path(self, relative_path: str,
                             output_path: Path,
                             collection_base_path: Optional[Path]) -> Optional[Path]:
        """Resolve a stored relative path back to an absolute path.

        This is the inverse of _make_relative_path. It tries multiple
        strategies to find the actual file.

        Args:
            relative_path: Relative path stored in database
            output_path: Current output directory being processed
            collection_base_path: Collection's local_path if available

        Returns:
            Absolute Path to file, or None if not found
        """
        # Priority 1: Try relative to collection base path
        if collection_base_path:
            full_path = collection_base_path / relative_path
            if full_path.exists():
                return full_path

        # Priority 2: Try relative to output_path (current workflow output)
        full_path = output_path / relative_path
        if full_path.exists():
            return full_path

        # Priority 3: If path starts with 'outputs/', try finding relative to that
        if relative_path.startswith('outputs/'):
            # Try stripping 'outputs/' prefix
            stripped = relative_path[len('outputs/'):]
            full_path = output_path / stripped
            if full_path.exists():
                return full_path

        # Priority 4: Legacy approach - try parent directories
        for i in range(1, 5):  # Try up to 4 parents
            try:
                parent = output_path
                for _ in range(i):
        ...
        # Priority 5: Try as absolute path (in case it was stored that way)
        abs_path = Path(relative_path)
        if abs_path.is_absolute() and abs_path.exists():
            return abs_path

        logger.debug(f"[PATH] Could not resolve path: {relative_path}")
        return None

    def _make_relative_path(self, output_file_path: Path,
                           collection_base_path: Optional[Path],
                           output_path: Path) -> str:
        """
        Create a relative path for storing in the database.

        Uses collection.local_path as the primary anchor for relative paths,
        with graceful fallbacks when that's not available.

        Args:
            output_file_path: Absolute path to the output file
            collection_base_path: Collection's local_path (preferred anchor)
            output_path: Current output directory (fallback anchor)

        Returns:
            Relative path string suitable for database storage
        """
        # Priority 1: Use collection.local_path as anchor
        if collection_base_path:
            try:
                rel_path = output_file_path.relative_to(collection_base_path)
                return str(rel_path)
            except ValueError:
                logger.debug(f"[PATH] File {output_file_path} is not under collection base {collection_base_path}")

        # Priority 2: Try to find 'outputs' directory in path hierarchy
        # This handles structure like: .../collection_id/outputs/date/workflow/item/file
        try:
            path_parts = output_file_path.parts
            for i, part in enumerate(path_parts):
                if part == 'outputs':
                    # Found outputs dir - make path relative to its parent (collection folder)
                    outputs_parent = Path(*path_parts[:i])
                    rel_path = output_file_path.relative_to(outputs_parent)
                    return str(rel_path)
        except (ValueError, IndexError):
            pass

        # Priority 3: Try relative to output_path
        try:
            rel_path = output_file_path.relative_to(output_path)
            return str(rel_path)
        except ValueError:
            pass

        # Final fallback: just use the filename
        logger.warning(f"[PATH] Could not create relative path for {output_file_path}, using filename only")
    ...
    def _resolve_target_item_id(self, source: str, step_name: str, output_type: str,
                                ...
                                output_scope: Optional[str] = None) -> Optional[str]:
        """
        Resolve the correct target item_id for a processing output.

        Rules (in priority order):
        1. If output_scope is "folder" -> use default_item_id (parent folder)
        2. If output_scope is "file" -> lookup source filename in item_map
        3. If output_scope is None -> use heuristic detection based on step/type patterns
        4. If item_map lookup fails, try to find/create file item (fallback)
        5. Final fallback to default_item_id if all else fails
        ...
        Args:
            ...
            output_scope: Explicit routing from plan definition:
                - "file": Route to individual source file
                - "folder": Route to parent folder
                - None: Use heuristic detection

        Returns:
            Resolved item_id for this output
        """
        # Use explicit output_scope from plan definition
        if output_scope == "folder":
            logger.debug(f"[RESOLVE] Folder scope: {output_type} -> {default_item_id}")
            return default_item_id
        elif output_scope == "file":
            # File scope - route to individual file via item_map lookup
            logger.debug(f"[RESOLVE] File scope: looking up {source}")
        else:
            # No output_scope defined - warn and default to folder (safe fallback)
            logger.warning(f"[RESOLVE] No output_scope defined for step '{step_name}' - defaulting to folder. Add output_scope to plan!")
            return default_item_id

        # File-level outputs: resolve to specific file item_id via item_map
        if item_map and source:
            ...
            # Look up the specific item_id for this source file with Unicode normalization
            if source_filename:
                logger.debug(f"[RESOLVE] Looking up source filename '{source_filename}' in item_map ({len(item_map)} entries)")
                # Use normalized lookup to handle Unicode variations (NFC vs NFD)
                target_item_id = self._normalized_filename_lookup(source_filename, item_map)
                if target_item_id:
                    logger.debug(f"[RESOLVE] File-level output: {source_filename} -> {target_item_id}")
                    return target_item_id

                # Fallback: try matching by stem (filename without extension) with normalization
                # This handles cases where source is .jpg but item_map has .tif
                source_stem = self._normalize_filename(Path(source_filename).stem)
                for item_filename, item_id_value in item_map.items():
                    item_stem = self._normalize_filename(Path(item_filename).stem)
                    if item_stem == source_stem:
                        logger.debug(f"[RESOLVE] File-level output (stem match): {source_filename} -> {item_id_value} (matched {item_filename})")
                        return item_id_value

                logger.warning(f"[RESOLVE] Source file '{source_filename}' not found in item_map (tried exact, normalized, and stem match)")
        ...
        # Check if we're assigning file-level output to folder (misrouting)
        if output_scope == "file":
            try:
                item = self.library_manager.storage.get_item(default_item_id)
                if item and item.type == 'folder':
                    logger.warning(f"[RESOLVE] ⚠️ File-level output ({output_type}) being assigned to folder item {default_item_id} - item_map lookup failed!")
            except Exception as e:
    ...
    def _ingest_processing_outputs(self, processing_result_id: str, collection_id: str,
        ...
        try:
            ...
            logger.info(f"[INGEST] Parameters: processing_result_id={processing_result_id}, collection_id={collection_id}, item_id={item_id}")

            # Log item_map status - CRITICAL for proper routing
            if item_map:
                logger.info(f"[INGEST] ✅ Item mapping available: {len(item_map)} files")
                # Log first few entries for debugging
                sample_entries = list(item_map.items())[:5]
                for filename, file_item_id in sample_entries:
                    logger.debug(f"[INGEST]   item_map: '{filename}' -> {file_item_id}")
                if len(item_map) > 5:
                    logger.debug(f"[INGEST]   ... and {len(item_map) - 5} more entries")
            else:
                logger.warning(f"[INGEST] ⚠️ No item_map provided - file-level outputs will fallback to folder item_id!")
                logger.warning(f"[INGEST] This may cause transcriptions to be assigned to the folder instead of individual files")

            # Validate required parameters
            if not collection_id or collection_id.strip() == "":
                logger.error(f"[INGEST] ❌ Missing or empty collection_id for processing result {processing_result_id}")
                logger.error(f"[INGEST] This WILL cause metadata association failures!")

            if not item_id or item_id.strip() == "":
                logger.warning(f"[INGEST] ⚠️ Missing or empty item_id (folder) for processing result {processing_result_id}")

            import json
            from fichero.director.workflow_manifest import WorkflowManifest

            # Get collection's local_path for relative path calculation
            ...
            if collection_id:
                try:
                    ...
                    if collection and collection.local_path:
                        collection_base_path = Path(collection.local_path)
                        logger.debug(f"[INGEST] Using collection local_path as base: {collection_base_path}")
                except Exception as e:
                    logger.warning(f"[INGEST] Could not retrieve collection local_path: {e}")
            ...
            # Extract library integration info (new in manifest v1.1)
            # This provides accurate item_id routing without guessing
            library_info = workflow_manifest.get('library', {})
            if library_info:
                manifest_item_id = library_info.get('item_id')
                manifest_collection_id = library_info.get('collection_id')
                manifest_item_map = library_info.get('item_map', {})

                # Use manifest values if available (they're authoritative)
                if manifest_item_id and not item_id:
                    item_id = manifest_item_id
                    logger.info(f"[INGEST] Using item_id from manifest: {item_id}")
                elif manifest_item_id and item_id and manifest_item_id != item_id:
                    logger.info(f"[INGEST] Overriding item_id with manifest value: {item_id} -> {manifest_item_id}")
                    item_id = manifest_item_id

                if manifest_collection_id and not collection_id:
                    collection_id = manifest_collection_id
                    logger.info(f"[INGEST] Using collection_id from manifest: {collection_id}")

                # Merge item_map from manifest (manifest values take precedence)
                if manifest_item_map:
                    if item_map:
                        # Merge: manifest values override
                        item_map = {**item_map, **manifest_item_map}
                        logger.info(f"[INGEST] Merged item_map from manifest: {len(manifest_item_map)} entries")
                    else:
                        item_map = manifest_item_map
                        logger.info(f"[INGEST] Using item_map from manifest: {len(item_map)} entries")
            else:
                logger.debug("[INGEST] No library info in manifest (pre-v1.1 or CLI processing)")
            ...
            # Track parse failures for setting partial status
            parse_failures = []
            total_lines_processed = 0
            total_outputs_attempted = 0
            ...
            for step_idx, step_record in enumerate(steps, 1):
                ...
                # Extract model info for workflow tracking
                step_model_info = step_record.get('model_info', {})
                # Extract output_scope from plan definition (authoritative routing)
                # Values: "file" (per-image), "folder" (parent folder), or None (use heuristic)
                step_output_scope = step_record.get('output_scope')

                logger.debug(f"[INGEST] Step {step_idx}/{len(steps)}: {step_name} (order={step_order}, status={step_status}, output_scope={step_output_scope})")
                        ...
                        for line_num, line in enumerate(f, 1):
                            ...
                            try:
                                ...
                                for output_idx, output_file in enumerate(outputs, 1):
                                    ...
                                    # Make path relative to collection folder using collection.local_path
                                    output_path_str = self._make_relative_path(
                                        output_file_path, collection_base_path, output_path
                                    )
                                    logger.debug(f"[INGEST] Relative path: {output_path_str}")

                                    # Determine the correct target item_id for this output
                                    # Pass output_scope from plan definition for authoritative routing
                                    target_item_id = self._resolve_target_item_id(
                                        ...
                                        output_scope=step_output_scope  # From plan definition
                                    )
                                    logger.debug(f"[INGEST] Resolved target_item_id: {target_item_id} (source: {source}, output_type: {output_type}, scope: {step_output_scope})")

                                    # Create ProcessingOutput record with model info
                                    processing_output = ProcessingOutput(
                                        ...
                                        is_valid=True,
                                        # Model/provider tracking from workflow_manifest.json
                                        model_name=step_model_info.get('model'),
                                        provider_name=step_model_info.get('provider'),
                                        model_version=step_model_info.get('model_version'),
                                        api_endpoint=step_model_info.get('api_endpoint'),
                                        api_parameters=json.dumps(step_model_info.get('api_parameters', {})) if step_model_info.get('api_parameters') else None,
                                        usage_stats=json.dumps(step_model_info.get('usage', {})) if step_model_info.get('usage') else None,
                                    )
            ...
            # Log and track parse failures
            if parse_failures:
                logger.warning(f"[INGEST] Parse failures during ingestion: {len(parse_failures)} errors")
                for failure in parse_failures[:5]:  # Log first 5 details
                    logger.warning(f"[INGEST]   {failure['step']}/{failure['manifest']} line {failure['line']}: {failure['error_type']} - {failure['error'][:100]}")
                if len(parse_failures) > 5:
                    logger.warning(f"[INGEST]   ... and {len(parse_failures) - 5} more errors")
            ...
            try:
                ...
                if db_outputs:
                    output_types = {}
                    item_ids_by_type = {}  # Track which item_ids got which output types
                    for output in db_outputs:
                        output_type = output.output_type
                        output_types[output_type] = output_types.get(output_type, 0) + 1
                        # Track item_id distribution
                        if output_type not in item_ids_by_type:
                            item_ids_by_type[output_type] = set()
                        item_ids_by_type[output_type].add(output.item_id)

                    logger.info(f"[INGEST] Output types summary: {dict(output_types)}")

                    # ROUTING VALIDATION: Check if file-level outputs are properly distributed
                    file_level_types = ['transcription', 'enhanced_image', 'prepared_image', 'cleaned_transcription']
                    for output_type in file_level_types:
                        if output_type in output_types and output_type in item_ids_by_type:
                            count = output_types[output_type]
                            unique_items = len(item_ids_by_type[output_type])
                            if count > 1 and unique_items == 1:
                                single_item = list(item_ids_by_type[output_type])[0]
                                logger.warning(f"[INGEST] ⚠️ ROUTING ISSUE: {count} '{output_type}' outputs all assigned to single item_id={single_item}")
                                logger.warning(f"[INGEST] This may indicate item_map lookup failures - transcriptions should go to individual images!")
                            elif count > 1 and unique_items == count:
                                logger.info(f"[INGEST] ✅ Routing OK: {count} '{output_type}' outputs distributed to {unique_items} unique items")
                    ...
                    # Store validation results and parse failures in processing result metadata
                    try:
                        ...
                        if processing_result and hasattr(processing_result, 'metadata'):
                            ...
                            if parse_failures:
                                processing_result.metadata['parse_failures'] = parse_failures
                                processing_result.metadata['parse_failure_count'] = len(parse_failures)

                            # Set partial status if there are issues
                            has_parse_failures = len(parse_failures) > 0
                            has_validation_issues = validation_result['completeness_score'] < 1.0
                            if has_parse_failures or has_validation_issues:
                                processing_result.status = 'partial'
                                logger.info(f"[INGEST] Setting ProcessingResult status to 'partial' due to: "
                                           f"parse_failures={has_parse_failures}, validation_issues={has_validation_issues}")
    ...
    def _determine_output_type(self, file_path: Path, step_name: str, plan_name: str = None) -> str:
        ...
        # Sanity check: Image files should NEVER be classified as catalogue/transcription/word_doc
        # This prevents misclassification when image files are in paths containing 'catalogue'
        image_extensions = {'.jpg', '.jpeg', '.png', '.tif', '.tiff', '.gif', '.bmp', '.webp'}
        if suffix in image_extensions:
            # Remove invalid classifications for image files
            invalid_for_images = {'catalogue', 'transcription', 'word_doc', 'json_data'}
            for invalid_type in invalid_for_images:
                if invalid_type in type_scores:
                    logger.debug(f"[OUTPUT_TYPE] Removing invalid type '{invalid_type}' for image file {file_path.name}")
                    del type_scores[invalid_type]
            # Ensure prepared_image has at least base score
            if 'prepared_image' not in type_scores:
                type_scores['prepared_image'] = 2
                logger.debug(f"[OUTPUT_TYPE] Added default prepared_image score for image file")
    ...
    def _extract_metadata_from_outputs(self, processing_result_id: str, collection_id: str,
        ...
        try:
            ...
            # Get collection's local_path for path resolution
            collection_base_path = None
            if collection_id:
                try:
                    collection = self.library_manager.storage.get_collection(collection_id)
                    if collection and collection.local_path:
                        collection_base_path = Path(collection.local_path)
                        logger.debug(f"[METADATA] Using collection local_path as base: {collection_base_path}")
                except Exception as e:
                    logger.warning(f"[METADATA] Could not retrieve collection local_path: {e}")
            ...
            for output_idx, output in enumerate(outputs, 1):
                try:
                    ...
                    # Reconstruct full path using multiple resolution strategies
                    full_output_path = self._resolve_output_path(
                        output.output_path, output_path, collection_base_path
                    )
                    logger.debug(f"[METADATA] Resolving full path: {full_output_path}")

                    if not full_output_path or not full_output_path.exists():
                        logger.warning(f"[METADATA] Output file not found, skipping: {full_output_path or output.output_path}")
                        outputs_skipped += 1
                        continue

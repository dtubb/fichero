...
class LibraryManager:
    ...
    async def delete_collection(self, collection_id: str) -> bool:
        ...
        try:
            ...
            # Remove local files/workspace if collection has local_path
            # This applies to:
            # - "local" collections: files copied into library
            # - "external" collections: workspace folder for outputs/cache
            # - "url" collections: workspace folder for outputs/cache
            if collection.local_path:
                local_path = Path(collection.local_path)
                if local_path.exists():
                    try:
                        shutil.rmtree(local_path)
                        logger.info(f"Removed collection files/workspace: {local_path}")
                    except Exception as file_error:
                        logger.warning(f"Could not remove files for collection {collection.name}: {file_error}")
    ...
    async def get_folder_items_recursive(self, folder_id: str, include_folders: bool = False) -> List[CollectionItem]:
        """Get all items within a folder recursively (including nested subfolders).

        Args:
            folder_id: ID of the folder (CollectionItem with type='folder')
            include_folders: If True, include folder items in results. If False, only return files.

        Returns:
            List of all items (files and optionally folders) within the folder hierarchy
        """
        try:
            # Get the folder item to find its collection_id
            folder = self.storage.get_item(folder_id)
            ...
            # Build a set of folder IDs to process (start with the target folder)
            folder_ids_to_process = {folder_id}
            processed_folder_ids = set()
            result_items = []

            # Iteratively find all nested folders and their items
            while folder_ids_to_process:
                current_folder_id = folder_ids_to_process.pop()
                if current_folder_id in processed_folder_ids:
                    continue
                processed_folder_ids.add(current_folder_id)

                # Find all items with this parent_id
                for item in all_items:
                    if item.parent_id == current_folder_id:
                        if item.type == "folder":
                            # Add folder to process queue for recursive descent
                            folder_ids_to_process.add(item.id)
                            if include_folders:
                                result_items.append(item)
                        else:
                            # Non-folder item - add to results
                            result_items.append(item)

            logger.info(f"Found {len(result_items)} items in folder {folder_id} (recursive)")
            return result_items

        except Exception as e:
            logger.error(f"Failed to get items in folder {folder_id}: {e}")
            return []

    async def get_folder_direct_children(self, folder_id: str) -> List[CollectionItem]:
        ...
        Returns:
            List of items that are direct children of this folder
        """
        try:
            # Get the folder item to find its collection_id
            folder = self.storage.get_item(folder_id)
            ...
            # Get all items in the collection and filter by parent_id
            all_items = self.storage.get_collection_items(folder.collection_id)
            children = [item for item in all_items if item.parent_id == folder_id]

            logger.info(f"Found {len(children)} direct children in folder {folder_id}")
            return children
    ...
    async def get_item_transcriptions(self, item_id: str,
                                        prefer_user_edits: bool = True) -> List[str]:
        """
        Get transcription texts for an item.

        If prefer_user_edits is True (default), returns user-edited transcription
        when available, falling back to AI-generated transcription.
        This enables auto-population of the preview pane with AI data.

        Args:
            item_id: Item ID
            prefer_user_edits: If True, prefer user_edit source, fallback to AI

        Returns:
            List of transcription text strings (user first, then AI if no user edit)
        """
        ...
        if not prefer_user_edits:
            # Simple case: return all transcriptions
            return [meta.value for meta in metadata_list]

        # Auto-populate mode: prefer user_edit, fallback to AI
        user_transcriptions = []
        ai_transcriptions = []

        for meta in metadata_list:
            if meta.source_label == 'user_edit':
                user_transcriptions.append(meta.value)
            else:
                ai_transcriptions.append(meta.value)

        # Return user edits if any exist, otherwise AI transcriptions
        if user_transcriptions:
            return user_transcriptions
        return ai_transcriptions

    async def get_item_catalogue_data(self, item_id: str,
                                       prefer_user_edits: bool = True) -> Dict[str, List[str]]:
        """
        Get catalogue data for an item organized by field.

        If prefer_user_edits is True (default), returns user edits when available,
        falling back to AI-generated data for fields without user edits.
        This enables auto-population of the preview pane with AI data.
        ...
        if not prefer_user_edits:
            # Simple case: return all metadata organized by field
            catalogue_data = {}
        ...
        # Auto-populate mode: prefer user_edit, fallback to AI
        # Group by field, separating user_edit from AI sources
        user_data = {}   # source_label == 'user_edit'
        ai_data = {}     # source_label != 'user_edit' (AI-generated)

        for metadata in metadata_list:
            if metadata.source_label == 'user_edit':
                if metadata.key not in user_data:
                    user_data[metadata.key] = []
                user_data[metadata.key].append(metadata.value)
            else:
                # AI-generated metadata (ai_qwen, ai_gpt, etc.)
                if metadata.key not in ai_data:
                    ai_data[metadata.key] = []
                ai_data[metadata.key].append(metadata.value)

        # Build final result: user_edit takes priority, AI fills gaps
        catalogue_data = {}

        # Get all unique keys
        all_keys = set(user_data.keys()) | set(ai_data.keys())

        for key in all_keys:
            if key in user_data and user_data[key]:
                # User has edited this field - use their value
                catalogue_data[key] = user_data[key]
            elif key in ai_data and ai_data[key]:
                # No user edit - auto-populate from AI
                catalogue_data[key] = ai_data[key]

        return catalogue_data

    # =========================================================================
    # Model Version Methods (for Adjust > Versions tab)
    # =========================================================================

    async def get_model_versions_for_item(self, item_id: str) -> List[Dict[str, Any]]:
        """Get all AI model versions for an item.

        Returns a list of model version summaries for display in the Versions tab.
        Each summary includes source_label, model_name, provider_name, created_at,
        schema_types, and record_count.
        ...
        """
        try:
            return self.storage.get_model_versions_for_item(item_id)
        except Exception as e:
            logger.error(f"Failed to get model versions: {e}")
            return []

    async def get_metadata_by_source(self, item_id: str, source_label: str,
                                     model_name: str = None) -> List['ExtractedMetadata']:
        """Get all metadata for an item from a specific source/model.

        Used to display the detail view when user clicks on a model version.
        ...
        """
        try:
            return self.storage.get_metadata_by_source(item_id, source_label, model_name)
        except Exception as e:
            logger.error(f"Failed to get metadata by source: {e}")
            return []

    async def get_field_versions(self, item_id: str, schema_type: str,
                                 key: str) -> List['ExtractedMetadata']:
        """Get all versions of a specific field.
        ...
        """
        try:
            return self.storage.get_field_versions(item_id, schema_type, key)
        except Exception as e:
            logger.error(f"Failed to get field versions: {e}")
            return []

    async def copy_metadata_to_user_pane(self, item_id: str,
                                         metadata_records: List['ExtractedMetadata']) -> bool:
        """Copy metadata from an AI version to the user's working copy.

        Creates or updates user_edit records for each provided metadata record.

        Args:
            item_id: Item ID
            metadata_records: List of ExtractedMetadata to copy
        ...
        try:
            ...
            for record in metadata_records:
                result = self.storage.update_extracted_metadata(
                    item_id=item_id,
                ...
                if not result:
                    success = False
                    logger.warning(f"Failed to copy {record.schema_type}.{record.key}")

            return success
        except Exception as e:
            logger.error(f"Failed to copy metadata to user pane: {e}")
            return False

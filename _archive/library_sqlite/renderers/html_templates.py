...
<body>
    ...
    <script>
        ...
        let magnifierZoom = 4;  // Default 4x magnification (range: 0.5x to 20x)
        ...
        function setMagnifierZoom(zoom) {{
            // Allow wide zoom range: 0.5x (zoom out) to 20x (extreme detail)
            magnifierZoom = Math.max(0.5, Math.min(20, zoom));
            // Format label: show decimal for fractional zooms, integer otherwise
            if (magnifierZoom < 1) {{
                magnifierZoomLabel.textContent = magnifierZoom.toFixed(1) + 'x';
            }} else if (magnifierZoom === Math.floor(magnifierZoom)) {{
        ...
        // Uses adaptive steps: 0.5 increments below 2x, 1.0 increments above
        function magnifierZoomIn() {{
            if (!magnifierActive) showMagnifier();
            if (magnifierZoom < 2) {{
                setMagnifierZoom(magnifierZoom + 0.5);
            }} else {{
        ...
        function updateMagnifier(mouseX, mouseY) {{
            ...
            // Use image's bounding rect to account for centering within wrapper
            const containerRect = container.getBoundingClientRect();
            const imgRect = img.getBoundingClientRect();

            // Mouse position in client coordinates
            const clientX = containerRect.left + mouseX;
            const clientY = containerRect.top + mouseY;

            // Calculate position relative to the actual image element
            const imageX = (clientX - imgRect.left) / scale;
            const imageY = (clientY - imgRect.top) / scale;
        ...
        // Scroll wheel on magnifier changes zoom level (adaptive increments)
        magnifier.addEventListener('wheel', function(e) {{
            e.preventDefault();
            // Use adaptive steps like the menu commands
            const step = magnifierZoom <= 2 ? 0.5 : 1;
            if (e.deltaY < 0) {{
                setMagnifierZoom(magnifierZoom + step);
            }} else {{
                setMagnifierZoom(magnifierZoom - step);
            }}
        }}, {{ passive: false }});

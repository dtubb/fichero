...
class FicheroDirector:
    ...
    def submit_task(self, folders: List[Path], plan_config: Dict, workflow_name: str = "default",
                   ...
                   library_item_map: Dict = None) -> str:
        """Submit a processing task

        Args:
            folders: List of prepared folder paths
            plan_config: Plan configuration
            workflow_name: Name of workflow to run
            variables: Additional variables for workflow
            priority: Task priority level
            document_context: Context from document window
            library_item_id: Library item_id of folder being processed (for metadata routing)
            library_collection_id: Library collection_id (for metadata routing)
            library_item_map: filename -> item_id mapping for file-level routing
        ...
        return self.task_manager.submit_task(
            folders, plan_config, workflow_name, variables, priority, document_context,

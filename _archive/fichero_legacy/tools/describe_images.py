...
def describe_batch(
    ...
    **kwargs
    ...
    for entry in manifest_entries:
        ...
        # Get paths to process - prefer 'outputs' field, fallback to 'source' or 'path'
        paths_to_process = []
        if 'outputs' in entry and entry['outputs']:
            ...
            paths_to_process.append(entry['source'])
        elif entry.get('path'):
            ...
            if source_folder:
                ...
                if not full_path.exists():
                    ...
                    if alt_path.exists():
                        ...
                        tool_logger.warning(f"⚠️ Image not found: {full_path} (also tried: {alt_path})")
                        continue
            else:
                full_path = path

            # Resolve symlinks and add to list
            if full_path.exists():
                full_path = full_path.resolve()
                image_paths.append(full_path)
            else:
                tool_logger.warning(f"⚠️ Image not found after resolution: {full_path}")

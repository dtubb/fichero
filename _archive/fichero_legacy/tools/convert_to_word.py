...
def convert_to_word_batch(
    ...
    **kwargs
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Use source images from the transcription manifest (which point to the prepared images)
        # The "outputs" field contains .txt filenames, not images, so we must use source
        # Images are in images_folder/documents/, so we add the documents prefix
        # (prepare_images stores outputs in assets/prepared/documents/[relative_path])
        processor = BatchProcessor(
            ...
            use_source=True,  # Use "source" field which contains image paths
            add_documents_prefix=True  # Add documents/ prefix since prepare_images outputs to documents/ subfolder
        )

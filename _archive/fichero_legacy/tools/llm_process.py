            ...
            if self.hierarchical:
                ...
                for folder in folders:
                    ...
                    if hasattr(self, 'manifest_proc'):
                        ...
                        # Note: llm_utils.py saves as _results.json, not _summary.json
                        self.manifest_proc.save_entry({
                            "source": folder.name,
                            "outputs": [f"{folder.name}_results.json"],
                            "type": "folder",
...
def process_documents_batch(
    ...
    **kwargs
    ...
    try:
        ...
        model_info = llm.get_model_info() if llm and hasattr(llm, 'get_model_info') else {}

        # Return basic stats with model info
        return {

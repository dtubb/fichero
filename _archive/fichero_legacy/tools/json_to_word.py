...
def convert_json_to_word(json_file_path: Path, output_path: Path, parent_folder: str) -> Dict:
    ...
    try:
        ...
        # Get results section - handle both wrapped {"results": {...}} and direct top-level structure
        # The LLM catalogue outputs data directly at top level, not wrapped in "results"
        results = data.get("results", data)  # Use data directly if no "results" wrapper
...
def json_to_word_batch(
    ...
    **kwargs
    ...
    processor = BatchProcessor(
        ...
        use_source=False,
        add_documents_prefix=False  # LLM catalogue outputs are at base level, not in documents/
    )

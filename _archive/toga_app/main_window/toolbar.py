...
from rubicon.objc import ObjCClass, SEL, objc_method, objc_property, NSObject, at
...
# Standard toolbar identifiers (built-in, FREE from macOS)
FLEX = "NSToolbarFlexibleSpaceItem"
SIDEBAR_SEPARATOR = "NSToolbarSidebarTrackingSeparatorItemIdentifier"
TOGGLE_SIDEBAR = "NSToolbarToggleSidebarItemIdentifier"  # Built-in sidebar toggle
...
TOOLBAR = (
    TOGGLE_SIDEBAR,  # Built-in sidebar toggle - FREE, works with NSSplitViewItem.sidebar
    SIDEBAR_SEPARATOR,  # Tracking separator - items left appear in sidebar area
    ...
    Button("toggleInspector:", "info.circle", "Inspector", "Toggle Inspector"),  # Native action
...
class _ToolbarDelegate(NSObject):
    """NSToolbar delegate with instance-level state."""

    # Instance properties (not class variables)
    _items = objc_property(object)
    _handler = objc_property(object)
    _on_search = objc_property(object)
    ...
    def toolbar_itemForItemIdentifier_willBeInsertedIntoToolbar_(
        ...
        items = self._items
        return items.get(str(identifier)) if items else None

    @objc_method
    def handleAction_(self, sender):
        handler = self._handler
        if handler and (method := getattr(handler, str(sender.itemIdentifier), None)):
            try:
                method()
    ...
    def handleMenu_(self, sender):
        handler = self._handler
        if handler and (method := getattr(handler, str(sender.representedObject), None)):
            try:
                method()
            except Exception as e:
    ...
    def searchChanged_(self, sender):
        ...
        if on_search:
            try:
                on_search(str(sender.stringValue))
            except Exception as e:
...
def _build(item, delegate):
    ...
    match item:
        case Button(id=id_, icon=icon, label=label, tooltip=tooltip):
            ...
            # Native actions (ending with :) use responder chain
            if id_.endswith(":"):
                ti.setTarget_(None)  # First responder handles it
                ti.setAction_(SEL(id_))
            else:
...
class AppToolbar:
    ...
    def __init__(self, handler, on_search: Optional[Callable[[str], None]] = None):
        # Create delegate with instance-level state
        self._delegate = _ToolbarDelegate.alloc().init()
        self._delegate._handler = handler
        self._delegate._on_search = on_search
        self._delegate._items = {}
        ...
        for item in TOOLBAR:
            ...
            if built := _build(item, self._delegate):
                self._delegate._items[item.id] = built
        ...
        logger.info(f"Toolbar created ({len(self._delegate._items)} items)")
    ...
    def attach_to_window_native(self, ns_window):
        """Attach directly to an NSWindow (for native window mode)."""
        try:
            ns_window.setToolbar_(self._toolbar)
            self._toolbar.setVisible_(True)
            logger.info("Toolbar attached to native window")
        except Exception as e:
            logger.error(f"Attach to native window failed: {e}")

    def set_enabled(self, id: str, enabled: bool):
        """Enable/disable item."""
        if item := self._delegate._items.get(id):

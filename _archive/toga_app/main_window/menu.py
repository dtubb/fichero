...
MENU_VIEW = Menu("View", (
    Item("Show Sidebar", "toggleSidebar:", "s", CTRL),  # Native action
    Item("Show Inspector", "toggleInspector:", "i", OPT),  # Native action
    SEP,
...
class AppMenu:
    ...
    def _build_item(self, item: Item) -> NSMenuItem:
        ...
        if item.handler.endswith(":"):
            mi = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
                _(item.label), SEL(item.handler), item.key or ""
            )
            mi.target = None  # First responder handles it
        else:
            mi = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
        ...
        if item.key:
            # Keyboard shortcuts default to CMD; item.mods adds extra modifiers

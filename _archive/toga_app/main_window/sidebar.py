...
With Document models (new data layer):
    from fichero.app.main_window.sidebar import SourceList, documents_to_sidebar_items
    from fichero.models import Document, DocType
    from fichero.db import db

    # Query collections from new data layer
...
_HIGHLIGHT_SOURCE_LIST = 1      # Sidebar-style selection (Finder/Mail look)
...
class _View(NSOutlineView):
    ...
    def outlineView_viewForTableColumn_item_(self, ov, col, w):
        ...
        if item.is_header:
            ...
            # Leading icon - uses system accent color (per HIG, avoid fixed colors)
            x = 4
            if item.icon:
                ...
                if img:
                    img.setTemplate_(True)  # System handles color based on selection
            ...
            # Trailing icon - uses system accent color
            trailing_x = 200 - 8
            if item.trailing_icon:
...
class SourceList:
    ...
    def __init__(
        ...
        self._view.selectionHighlightStyle = _HIGHLIGHT_SOURCE_LIST
        self._view.floatsGroupRows = False
        self._view.backgroundColor = NSColor.clearColor
        self._view.drawsBackground = False  # No need to draw clear background
        self._view.wantsLayer = True
        self._view.draggingDestinationFeedbackStyle = _DRAG_FEEDBACK_REGULAR

        # Auto-save expanded items - FREE from macOS
        self._view.autosaveExpandedItems = True
        self._view.autosaveName = "FicheroSidebar"
    ...
    def select(self, item_id: str) -> bool:
        ...
        Expands parent items if necessary to make the item visible.
        """
        # Find the item in our hierarchy to get its parents
        target = self.find(item_id)
        if not target:
            return False

        # Expand parent items first (find and expand the path to this item)
        for root in self._items:
            if self._expand_path_to(root, item_id):
                break
        ...
        if w := self._cache.get(item_id):
            ...
            if row >= 0:
                ...
                self._view.scrollRowToVisible_(row)
                return True
        return False

    def _expand_path_to(self, item: SourceListItem, target_id: str) -> bool:
        """Expand items along the path to target_id. Returns True if found."""
        if item.id == target_id:
            return True
        for child in item.children:
            if self._expand_path_to(child, target_id):
                # Found in this subtree - expand this item
                if w := self._cache.get(item.id):
                    self._view.expandItem_(w)
                return True
        return False
...
def documents_to_sidebar_items(
    documents: list,
    group_by_type: bool = True
) -> list[SourceListItem]:
    """Convert Document models to SourceListItem hierarchy.

    Creates a sidebar structure from Document models, optionally
    grouping by type (local vs external collections).

    Args:
        documents: list[Document] - Documents with doc_type=collection
        group_by_type: If True, group into sections by collection type

    Returns:
        list[SourceListItem] ready for SourceList.items
    ...
    if not group_by_type:
        ...
        return [
            SourceListItem(
                ...
                text=doc.name,
                icon="folder.fill",
                data=doc,  # Store the Document directly
                accepts_drops=True,
            )
            for doc in documents
        ]

    # Group by source_type (from metadata)
    local_items = []
    external_items = []

    for doc in documents:
        source_type = doc.metadata.get("source_type", "local") if doc.metadata else "local"

        # Count children (for badge)
        child_count = db.count(type(doc), parent_id=doc.id)
        badge = str(child_count) if child_count > 0 else None

        # Get children (folders) for hierarchy
        children_docs = db.query(type(doc), parent_id=doc.id, doc_type=DocType.folder)
        children = [
            SourceListItem(
                id=child.id,
                text=child.name,
        ...
        item = SourceListItem(
            id=doc.id,
            text=doc.name,
            icon="folder.fill" if source_type == "local" else "link",
            badge=badge,
            data=doc,  # Store the Document directly
            accepts_drops=True,
            children=children,
    ...
    if local_items:
        sections.append(SourceListItem(
            id="local",
            text="COLLECTIONS",
            is_header=True,
            children=local_items,
        ))
    ...
    if external_items:
        sections.append(SourceListItem(
            ...
            text="EXTERNAL",
            is_header=True,
            children=external_items,
        ))

    return sections


def document_to_sidebar_item(doc) -> SourceListItem:
    """Convert a single Document to a SourceListItem.

    Args:
        doc: Document model

    Returns:
        SourceListItem with the document stored in .data
    """
    from fichero.db import db

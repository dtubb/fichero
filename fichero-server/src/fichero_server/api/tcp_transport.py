...
The sibling of ``fichero_server.api.uds_transport``. Read them together: they are the
...
``fichero_server.api.main:app`` object (``uds_transport`` is middleware, not a second
...
reaches for the bare ``fichero_server.api.main:app`` silently loses it — no error, no
...
# (`uvicorn fichero_server.api.tcp_transport:app --host ... --port ...`).

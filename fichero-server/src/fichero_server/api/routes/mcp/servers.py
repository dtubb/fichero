...
from fichero_server.models import MCPServer
# NOTE: fichero_server.mcp.manager is imported inside the handlers, not here (#3950).
...
# import; a real (non-string) annotation here would defeat the whole point.
from typing import TYPE_CHECKING

if TYPE_CHECKING:  # pragma: no cover - import-time typing only
    from fichero_server.mcp.manager import MCPServerConfig
...
def get_mcp_manager():
    ...
    itself moves off the startup path (#3950). fichero_server.mcp.manager pulls
    langchain_mcp_adapters -> mcp + langgraph; nothing here needs it until a
    request actually touches an MCP server.
    """
    from fichero_server.mcp.manager import get_mcp_manager as _get_mcp_manager
...
async def load_mcp_tools_into_workflow_registry(
    ...
    try:
        ...
        from fichero_server.workflows.tools.mcp import load_mcp_tools_into_registry
...
async def reload_mcp_tools_in_workflow_registry(
    ...
    try:
        ...
        # Reload MCP tools
        from fichero_server.workflows.tools.mcp import reload_mcp_tools

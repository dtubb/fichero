...
# NOTE: fichero_server.llm is imported inside the one handler that prices a model
...
from fichero_server.models import ReinstallDefaultWorkflowsResponse
...
# `fichero_server.api.routes.workflow.workflows.<name>`. Two things must both hold:
...
def get_model_cost(*args, **kwargs):
    """Passthrough to fichero_server.llm.get_model_cost; imports it on first call (#3950)."""
    from fichero_server.llm import get_model_cost as _impl  # noqa: PLC0415
...
def _dict_to_node_def(node_dict: dict, enrich_ports: bool = True) -> NodeDef:
    ...
    """
    from fichero_server.workflows.types import InputMapping, OutputSchema

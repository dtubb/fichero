...
Usage:
    # Direct
    uvicorn fichero_server.api.main:app --reload
...
def _resolve_engine_version() -> str:
    ...
    The Python project is named ``fichero-server``, but Briefcase bundles it as
    the ``server`` app and ships only ``server-<ver>.dist-info`` — so looking up
    one name alone made the shipped app report ``dev``. Try each distribution
    name (dev/editable installs carry ``fichero-server``, legacy venvs ``fichero``;
    the Briefcase bundle carries ``server``) before falling back to ``dev``.
    """
    for dist_name in ("fichero-server", "server", "fichero", "engine"):
        try:
...
def _ensure_bootstrap_token_written() -> None:
    ...
    if not _auth_enabled():
        return
    from fichero_server.api.auth import initialize_token, sync_app_bootstrap_token
...
async def lifespan(app: FastAPI):
    ...
    def _warm_workflow_stack() -> None:
        try:
            ...
            from fichero_server.workflows.registry import _ensure_tools_loaded
...
if os.environ.get("FICHERO_DISABLE_AUTH", "").lower() not in {"1", "true", "yes"}:
    from fichero_server.api.auth import attach_auth_middleware, initialize_token
...
async def get_stats(db: Database = Depends(get_library_database)):
    ...
    from fichero_server.models import Document, Artifact
...
# `from fichero_server.api.routes import <name>` — but importing them here as well
...
from fichero_server.api.routes.auth import (  # noqa: E402
    accounts as auth_accounts,
    authz,
    pairing,
    sandbox_access,
)
from fichero_server.api.routes.citation import (  # noqa: E402
    bibliography,
    citations,
    references,
...
from fichero_server.api.routes.claim import (  # noqa: E402
    claims,
    curation as claim_curation,
    links as claim_links,
)
from fichero_server.api.routes.document import (  # noqa: E402
    annotations,
    artifacts,
    classifications,
...
from fichero_server.api.routes.ingest import (  # noqa: E402
    export,
    iiif,
    image_editing,
)
from fichero_server.api.routes.interpretation import (  # noqa: E402
    canvas,
    hermeneutics,
)
from fichero_server.api.routes.library import (  # noqa: E402
...
from fichero_server.api.routes.research import agents as research_agents  # noqa: E402
from fichero_server.api.routes.search import (  # noqa: E402
    explain as search_explain,
)
from fichero_server.api.routes.system import (  # noqa: E402
    actions,
    actions_registry,
    activity,
    agent_memory,

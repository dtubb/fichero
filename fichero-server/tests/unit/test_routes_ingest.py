...
class TestIngestFile:
    def test_route_ingest_writes_audit_and_emits_document_created(self, client, db, tmp_path, monkeypatch):
        ...
        with patch("fichero_server.importers.ingest.ingest_file", return_value=_make_document()):
...
class TestIngestFolder:
    def test_ingest_folder_returns_task_id(self, client, tmp_path):
        ...
        with patch("fichero_server.importers.ingest.ingest_folder") as _mock_folder, \
             patch("fichero_server.importers.ingest.count_files", return_value=1), \
             patch("fichero_server.importers.ingest.IngestMode"):
    ...
    def test_background_ingest_uses_fresh_db_handle(self, client, tmp_path):
        ...
        with patch("fichero_server.importers.ingest.ingest_folder", side_effect=_fake_ingest_folder), \
             patch("fichero_server.importers.ingest.count_files", return_value=1), \
             patch("fichero_server.importers.ingest.IngestMode"), \
             patch("fichero_server.api.routes.ingest.db_manager.get_database", return_value=fresh_db):
            r = client.post("/api/ingest/folder", json={"path": str(tmp_path)})
    ...
    def test_per_file_events_fire_only_after_the_document_commit(
        ...
        monkeypatch.setattr("fichero_server.api.change_stream.emit_change", capture_emit)
        with patch("fichero_server.importers.ingest.ingest_folder", side_effect=fake_ingest_folder), \
             patch("fichero_server.importers.ingest.count_files", return_value=2):
...
class TestIngestStatus:
    ...
    def test_task_status_after_folder_ingest(self, client, tmp_path):
        ...
        with patch("fichero_server.importers.ingest.ingest_folder"), \
             patch("fichero_server.importers.ingest.count_files", return_value=2), \
             patch("fichero_server.importers.ingest.IngestMode"):

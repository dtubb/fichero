...
class TestDatabaseBasics:
    ...
    def test_default_path(self, monkeypatch):
        ...
        import fichero_server.db.migrations.schema as _db_migrations
        # #4983 phase 1: every migration now takes an optional `failures`
        # list as a second positional arg — these stubs accept (and ignore)
        # it so `Database.__init__`'s real call shape keeps working.
        monkeypatch.setattr(_db_migrations, "migrate_document_table", lambda _conn, _f=None: None)
        monkeypatch.setattr(_db_migrations, "migrate_workflow_table", lambda _conn, _f=None: None)
        monkeypatch.setattr(_db_migrations, "migrate_saved_search_table", lambda _conn, _f=None: None)
        monkeypatch.setattr(_db_migrations, "migrate_provider_refs_table", lambda _conn, _f=None: None)
        monkeypatch.setattr(_db_migrations, "migrate_known_libraries_table", lambda _conn, _f=None: None)
        monkeypatch.setattr(_db_migrations, "migrate_library_entity_types_table", lambda _conn, _f=None: None)
        monkeypatch.setattr(_db_migrations, "migrate_canvas_layout_table", lambda _conn, _f=None: None)
...
class TestModelCRUD:
    ...
    def test_save_and_get_model(self, temp_db):
        ...
        from fichero_server.models import Provider, Model, ProviderType
...
class TestToolCRUD:
    ...
    def test_save_and_get_tool(self, temp_db):
        ...
        from fichero_server.models import Tool
...
class TestWorkflowFold:
    ...
    def test_reseeding_resurrects_soft_deleted_mirror(self, temp_db):
        """Pre-lock databases may hold soft-deleted mirror rows (deletes were
        only blocked once documents enforced read_only). Re-saving the
        workflow must bring its mirror back to life, or presets silently
        vanish from the tree forever."""
        workflow = Workflow(name="Preset", is_system=True)
        temp_db.save(workflow)
        ...
        resurrected = temp_db.get(Document, workflow.id)
...
class TestEmbeddingsModelLoading:
    ...
    def test_embedder_uses_managed_cache_dir(self, temp_db, monkeypatch):
        ...
        import sys
        import types
        import fichero_server.db.embeddings as db_embeddings_module
...
class TestFuzzyMatch:
    ...
    def test_exact_match_still_works(self):
        import pandas as pd
        from fichero_server.db import _fuzzy_contains_any_term, _fold_for_search
    ...
    def test_substring_fallback(self):
        import pandas as pd
        from fichero_server.db import _fuzzy_contains_any_term

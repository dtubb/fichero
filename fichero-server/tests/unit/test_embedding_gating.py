...
from fichero_server.llm.embeddings import _enforce_embedding_call_allowed
...
def test_remote_embedding_allowed_when_fallbacks_disabled() -> None:
    ...
    with (
        patch("fichero_server.llm._is_local_or_builtin_provider", return_value=False),
        patch("fichero_server.llm.is_local_only", return_value=False),
...
def test_remote_embedding_blocked_when_local_only() -> None:
    ...
    with (
        patch("fichero_server.llm._is_local_or_builtin_provider", return_value=False),
        patch("fichero_server.llm.is_local_only", return_value=True),
    ):
...
def test_local_provider_always_allowed() -> None:
    ...
    with (
        patch("fichero_server.llm._is_local_or_builtin_provider", return_value=True),
        patch("fichero_server.llm.is_local_only", return_value=True),  # even local-only ON
    ):
        # Must not raise

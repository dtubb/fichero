...
class TestChatStructuredDispatch:
    ...
    async def test_apple_provider_routes_to_fm_bridge(self):
        ...
        with patch(
            "fichero_server.llm._apple_intelligence_structured",
    ...
    async def test_omlx_uses_provider_default_structured_method(self):
        ...
        with patch("fichero_server.llm.get_langchain_model", return_value=base_model), patch(
            "fichero_server.llm._ensure_managed_local_provider_ready",
            new=AsyncMock(),
        ):
    ...
    async def test_include_raw_dict_unwraps_to_parsed(self, caplog):
        ...
        with patch("fichero_server.llm.get_langchain_model", return_value=base_model), \
             caplog.at_level(logging.INFO, logger="fichero_server.llm"):
    ...
    async def test_collect_usage_captures_apple_estimate(self):
        ...
        from fichero_server.llm import collect_usage, _log_apple_usage_estimate
    ...
    async def test_apple_logs_estimated_usage(self, caplog):
        ...
        log line (estimated)."""
        import logging
        from fichero_server.llm import _log_apple_usage_estimate

        cfg = LLMConfig(provider="apple", model="apple-intelligence")
        with caplog.at_level(logging.INFO, logger="fichero_server.llm"):
    ...
    async def test_apple_usage_prefers_bridge_usage_payload(self):
        ...
        from fichero_server.llm import _log_apple_usage_from_bridge, collect_usage
...
class TestProviderQuotaHandling:
    ...
    async def test_chat_structured_raises_typed_quota_error_once(self):
        ...
        tracker = MagicMock()
        with patch("fichero_server.llm.get_langchain_model", return_value=base_model), \
             patch("fichero_server.workflows.activity.get_activity_tracker", return_value=tracker), \
             patch("fichero_server.llm.logger.warning") as warn:
...
class TestChatStructuredWithFallback:
    ...
    async def test_passthrough_when_apple_succeeds(self):
        cfg = LLMConfig(provider="apple", model="apple-intelligence")
        with patch(
            "fichero_server.llm.chat_structured",
    ...
    async def test_schema_failure_retries_without_schema_prompt_injection(self):
        ...
        with patch("fichero_server.llm.chat_structured", new=mock_structured), \
             patch("fichero_server.llm.resolve_model_alias") as mock_resolve:
            result = await chat_structured_with_fallback(
    ...
    @pytest.mark.asyncio
    async def test_retryable_decode_retries_once_on_the_SAME_model(self):
        """Ruling A (Daniel 2026-09-07): a transient grammar-decode miss re-rolls
        the SAME selected model once — NOT a different model, and no alias walk."""
        apple_cfg = LLMConfig(provider="apple", model="apple-intelligence")
        good = _Result(answer="from-retry")
        mock_structured = AsyncMock(
            side_effect=[
                StructuredDecodeError("(decoding): missed", kind="decoding"),
                good,
            ]
        )
        with patch("fichero_server.llm.chat_structured", new=mock_structured), \
        ...
        assert result == good
        assert mock_structured.await_count == 2
        # Same model both times — the config is unchanged, no substitution.
        for call in mock_structured.await_args_list:
            cfg = call.kwargs.get("config") or call.args[2]
            assert (cfg.provider, cfg.model) == ("apple", "apple-intelligence")
        mock_resolve.assert_not_called()

    @pytest.mark.asyncio
    async def test_retryable_decode_that_fails_twice_raises_loud(self):
        """One retry only — a second decode failure propagates loud, never a
        fall to a different model."""
        apple_cfg = LLMConfig(provider="apple", model="apple-intelligence")
        mock_structured = AsyncMock(
            side_effect=[
                StructuredDecodeError("(decoding): missed", kind="decoding"),
                StructuredDecodeError("(decoding): missed again", kind="decoding"),
            ]
        )
        with patch("fichero_server.llm.chat_structured", new=mock_structured), \
             patch("fichero_server.llm.resolve_model_alias") as mock_resolve:
            with pytest.raises(StructuredDecodeError, match="missed again"):
    ...
    @pytest.mark.asyncio
    async def test_guardrail_raises_loud_and_never_calls_a_second_model(self):
        """The ladder is gone: a guardrail refusal propagates loud after ONE
        call, never a fall to a different or paid model."""
        apple_cfg = LLMConfig(provider="apple", model="apple-intelligence")
...
class TestSupportsLocale:
    ...
    def setup_method(self):
        ...
        from fichero_server.llm import _LOCALE_SUPPORT_CACHE
...
class TestOpenRouterStripParallelToolUse:
    ...
    async def test_strips_parallel_tool_calls_field(self):
        ...
        from fichero_server.llm import _openrouter_strip_parallel_tool_use

...
from fichero_server.api.main import _is_allowed_local_path
...
class TestWiredIntoTheRealCheck:
    ...
    def test_the_allow_list_consults_the_drop_helper(self):
        with patch.object(Path, "home", staticmethod(lambda: HOME)):
            # The authority moved to fichero_server.security.path_security (#4230);
            # fichero_server.api.main re-exports it under the old private name.
            with patch(
                "fichero_server.security.path_security.is_sandbox_container_drop_staging"
            ) as helper:

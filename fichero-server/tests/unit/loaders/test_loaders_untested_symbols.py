...
from fichero_server.loaders.pdf_loader import PDFLoader, PDFTextLoader
from fichero_server.loaders.unified import UnifiedLoader
...
def test_public_loader_exports_survive_specialized_imports() -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import fichero_server.loaders.document_loader; import fichero_server.loaders.pdf_loader; "
            "from fichero_server.loaders import UnifiedLoader, load_media",
        ],
        capture_output=True,
        text=True,
        check=False,
...
async def test_unified_loader_load_sync_uses_image_hint(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("fichero_server.loaders.unified.ImageLoader", HintImageLoader)
    loader = UnifiedLoader()
...
def test_parse_xmp_sidecar_calls_parser_for_present_file(tmp_path):
    ...
    with patch("fichero_server.loaders.xmp_loader._parse_xmp_file") as parse_fn:

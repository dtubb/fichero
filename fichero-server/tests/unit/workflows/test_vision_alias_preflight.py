...
def test_spanish_script_subworkflow_parent_is_valid_and_passes_preflight(monkeypatch):
    ...
    path = (
        ...
        / "src/fichero_server/resources/default_workflows/transcribe_spanish_script_v2_subworkflow.json"
    )
...
def test_spanish_script_v2_child_resolves_distinct_vision_tiers(monkeypatch):
    ...
    path = (
        Path(__file__).parents[3]
        / "src/fichero_server/resources/default_workflows/transcribe_spanish_script_v2_child.json"
    )
...
def test_paleography_ensemble_without_optional_translation_passes_preflight(monkeypatch):
    ...
    path = (
        Path(__file__).parents[3]
        / "src/fichero_server/resources/default_workflows/transcribe_paleography_ensemble.json"
    )
    data = json.loads(path.read_text())

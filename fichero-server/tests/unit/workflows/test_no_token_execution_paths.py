...
def test_transcribe_executes_digital_text_selection_without_external_calls(
    ...
    monkeypatch.setattr("fichero_server.workflows.tools.transcribe.process_vision", fake_process_vision)
...
@pytest.mark.parametrize(
    ...
    [
        ("audio_transcribe", "fichero_server.workflows.tools.audio_transcribe.process_audio", "/library/interview.mp3", FileType.audio),
        ("video_describe", "fichero_server.workflows.tools.video_describe.process_video", "/library/film.mov", FileType.video),
    ],
)
...
def test_transcribe_htr_runs_without_tokens_and_keeps_page_scope(
    ...
    monkeypatch.setattr("fichero_server.workflows.tools.transcribe.process_vision", fake_process_vision)
    monkeypatch.setattr(
        "fichero_server.workflows.tools.transcribe_review.process_vision",
        fake_process_vision,
    )
    monkeypatch.setattr(
...
def test_catalogue_full_pipeline_runs_from_folder_with_stubs(
    ...
    monkeypatch.setattr(
        "fichero_server.workflows.tools.kg_persist_finalize._vector_row_count",
        lambda *args, **kwargs: 0,
    )
    monkeypatch.setattr(
        "fichero_server.db.Database.embed_entities",
        lambda self, entities: 0,
    )
    monkeypatch.setattr(
        "fichero_server.db.Database.embed_claims",
        lambda self, claims: 0,
    )
    monkeypatch.setattr(
        "fichero_server.db.Database._embed_texts",

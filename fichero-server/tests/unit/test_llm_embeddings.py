...
from fichero_server.llm import embeddings as llm_embeddings
...
def test_remote_embeddings_allowed_when_paid_fallbacks_disabled(monkeypatch) -> None:
    ...
    with patch("fichero_server.llm._paid_remote_fallbacks_enabled", return_value=False):
        with patch(
            "fichero_server.llm.embeddings._get_langchain_embeddings",
...
def test_remote_embeddings_run_when_paid_fallbacks_enabled(monkeypatch) -> None:
    ...
    with patch("fichero_server.llm._paid_remote_fallbacks_enabled", return_value=True):
        with patch(
            "fichero_server.llm.embeddings._get_langchain_embeddings",
            return_value=FakeEmbeddings(),
        ):

...
from fichero_server.llm import _FM_BRIDGE_MISSING_MESSAGE, probe_apple_intelligence_bridge
...
async def test_probe_apple_bridge_returns_unavailable_reason_without_spawning(
    ...
    monkeypatch.setattr(
        "fichero_server.llm._fm_bridge_unavailable_reason",
        lambda: "Apple Intelligence is not available on this device",
    )
    monkeypatch.setattr(
        "fichero_server.llm._find_fm_bridge_binary",
...
async def test_probe_apple_bridge_reports_missing_binary(
    ...
    monkeypatch.setattr("fichero_server.llm._fm_bridge_unavailable_reason", lambda: None)
    monkeypatch.setattr("fichero_server.llm._find_fm_bridge_binary", lambda: None)
...
async def test_probe_apple_bridge_reports_subprocess_launch_failure(
    ...
    monkeypatch.setattr("fichero_server.llm._fm_bridge_unavailable_reason", lambda: None)
    monkeypatch.setattr("fichero_server.llm._find_fm_bridge_binary", lambda: "/tmp/fm-bridge")

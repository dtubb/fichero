...
suite (`pytest fichero-server/tests/unit/test_check_mac_app_store_target.py`).
"""
...
def test_engine_embed_input_file_list_tracks_source_and_preflight_files() -> None:
    ...
    assert any(entry.startswith("$(SRCROOT)/../fichero-server/src/fichero_server/") for entry in entries)

...
from fichero_server.workflows.tools.multi_agent import (
    supervisor_agent,
    swarm_agent,
    agent_coordinator,
)
...
class TestSupervisorWorkflow:
    ...
    async def test_supervisor_delegates_to_workers(self, llm_config, mock_state):
        ...
        with patch(
            "fichero_server.workflows.tools.multi_agent.chat_workflow",
            side_effect=mock_supervisor_response,
        ):
            with patch(
                "fichero_server.workflows.tools.multi_agent._run_agent_loop",
                AsyncMock(
...
class TestModelComparisonWorkflow:
    ...
    async def test_comparison_in_workflow(self, llm_config, mock_state):
        ...
        with patch(
            "fichero_server.workflows.model_comparison.chat_workflow",

...
from fichero_server.api.auth import prepare_app_bootstrap_token_for_launch
...
import sys
from fichero_server.api.transport_diagnostics import describe_transport, transport_banner
path = sys.argv[1]
...
' "$FICHERO_UDS_PATH"
  exec env PYTHONPATH="$API_ROOT/src" "$PYTHON_BIN" -m uvicorn \
    fichero_server.api.uds_transport:app --uds "$FICHERO_UDS_PATH" \

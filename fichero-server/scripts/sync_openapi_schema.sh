...
# Sync OpenAPI schema from fichero-server into the Swift package imported by the SwiftUI app.
# Run from repo root: ./fichero-server/scripts/sync_openapi_schema.sh
#
...
if [ -n "${FICHERO_PYTHON_BIN:-}" ] && [ -x "${FICHERO_PYTHON_BIN}" ]; then
  ...
  echo "⚠️  Using fichero-server/.venv — if the engine is not installed there," >&2

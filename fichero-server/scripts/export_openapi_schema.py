...
Output:
    fichero-server/tests/contracts/openapi.json - Full OpenAPI 3.0 schema
    fichero-server/tests/contracts/endpoints.json - Simplified endpoint list for Swift validation
...
def build_openapi_schema() -> dict:
    ...
    """
    # Imported HERE, not at module scope: constructing the app is the expensive
    # part of this script, and `--bake-identity-from` must not pay for it (#5047).
...
CONTRACT_IDENTITY_PATH = (
    Path(__file__).parent.parent / "src" / "fichero_server" / "api" / "contract_identity_generated.py"
)


def write_contract_identity(contract_bytes: bytes, path: Path = CONTRACT_IDENTITY_PATH) -> str:
    """Bake the contract's identity into the engine package (#5047).

    The engine must be able to state, at connect time, WHICH contract its wire
    speaks — so a remote app can refuse a pairing when the two builds disagree
    (spec `contract.runtime-compatibility`, ruling 1).

    It cannot compute that at runtime. `app.openapi()` is the OpenAPI 3.1
    document for whatever `FICHERO_FEATURE_TIER` is live (default `release`);
    the contract is this script's post-processed 3.0.3 output exported at the
    `dev` tier, with split variants injected and nullables down-converted. The
    two documents are different by construction, so a runtime hash of the served
    schema could never match the identity the Swift client bakes from this file.
    Hashing the bytes we just wrote is the only value both ends can agree on.

    Written from the SAME bytes as `openapi.json`, in the same run, so the two
    cannot drift — the failure mode #5046 was about.

    Returns the digest so callers can report it.
    """
    digest = hashlib.sha256(contract_bytes).hexdigest()
    version = json.loads(contract_bytes)["info"]["version"]
    content = (
...
def write_swift_contract_identity(
    contract_bytes: bytes, path: Path = SWIFT_CONTRACT_IDENTITY_PATH
) -> str:
    """Bake the SAME identity into the Swift client (#5047, client half).

    Written in the same pass, from the same bytes, as the engine's Python
    identity — which is the whole point. If the two were generated by separate
    steps they could be generated from different documents, and the mismatch the
    check exists to catch would be produced BY the check's own plumbing.

    The digest is over `tests/contracts/openapi.json`, which `sync_openapi_
    schema.sh` then `cp`s to the Swift package verbatim, so the file this client
    is generated from and the bytes hashed here are the same bytes.
    """
    digest = hashlib.sha256(contract_bytes).hexdigest()
    version = json.loads(contract_bytes)["info"]["version"]
    content = (
        ...
        f'    public static let sha256 = "{digest}"\n'
        "}\n"
    )
    if not path.exists() or path.read_text() != content:
        path.write_text(content)
...
def main():
    # #5047: re-bake both identities from an EXISTING contract without exporting.
    # `sync_openapi_schema.sh` needs this on its abort path: when the version
    # guard rejects a sync it restores the previous `openapi.json`, and the
    # identities must be restored with it or they would describe a document that
    # was rolled back — a partial write that every other guard reports as green,
    # which is the #5046 failure shape exactly.
    if "--bake-identity-from" in sys.argv:
        source = Path(sys.argv[sys.argv.index("--bake-identity-from") + 1])
        if not source.is_file():
            print(f"✗ Cannot bake contract identity: {source} is not readable", file=sys.stderr)
            return 1
        contract_bytes = source.read_bytes()
        if not contract_bytes:
            print(f"✗ Cannot bake contract identity: {source} is empty", file=sys.stderr)
            return 1
        digest = write_contract_identity(contract_bytes)
        write_swift_contract_identity(contract_bytes)
        print(f"✓ Re-baked contract identity from {source} ({digest[:12]}…)")
        return 0
    ...
    # #5047: bake the identity from the bytes just written, never from the dict
    # in memory — the file is what every other copy of the contract is `cp`ed
    # from, so it is the only thing both ends can hash identically.
    contract_bytes = openapi_path.read_bytes()
    digest = write_contract_identity(contract_bytes)
    # BOTH ends in the same pass, from the same bytes (#5047). Generating them
    # separately would let the plumbing manufacture the mismatch it detects.
    write_swift_contract_identity(contract_bytes)
    print(f"✓ Contract identity baked for engine + client ({digest[:12]}…)")

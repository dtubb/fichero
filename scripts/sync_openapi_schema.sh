...
#
# This updates:
#   - tests/contracts/openapi.json (for testing)
#   - tests/contracts/endpoints.json (for validation)
#   - Fichero/FicheroAPIClient/Sources/FicheroAPIClient/openapi.json (for code generation)
...
cd "$REPO_ROOT"
PYTHONPATH=src .venv/bin/python scripts/export_openapi_schema.py

echo ""
echo "Copying schema to Swift package..."
cp tests/contracts/openapi.json Fichero/FicheroAPIClient/Sources/FicheroAPIClient/openapi.json

echo ""
echo "Rebuilding Swift API client..."
cd Fichero/FicheroAPIClient

...
class BatchProcessor:
    ...
    def process(self) -> Dict:
        ...
        for doc in self.input_proc.stream_entries():
            ...
            elif doc.get("input_path"):  # Use input_path from manifest
                paths_to_process.append(doc["input_path"])
    ...
    def _process_batch(self, batch: List[dict], stats: dict, progress, task):
        ...
        for doc in batch:
            try:
                ...
                if self.base_folder:
                    # Base folder should be the project root, so we need to add documents/

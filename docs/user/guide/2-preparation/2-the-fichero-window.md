...
1. **Toolbar** — the row of controls across the top: import, the layout picker, running a workflow on your selection, the search field, and the sidebar and inspector toggles. *See [The Toolbar](#the-toolbar) below, and [Appendix C: Keyboard Shortcuts](../8-appendixes/4-appendix-c-keyboard-shortcuts.md) for the full command list.*
...
3. **Library Browser** — the documents and folders of the current view, shown in whichever layout you have chosen. *See [The Library Browser](#the-library-browser) below, and [Managing Your Library](3-managing-your-library.md).*
4. **Reader** — the page, scan, or PDF you are looking at, with its own tabs for the page, its knowledge, and your notes. *See [The Reader](#the-reader) below, and [Reading, Transcribing, and Annotating](../4-reading/1-reading-transcribing-and-annotating.md).*
5. **Inspector** — the panel that shows everything Fichero knows about the selected document: its source text, artifacts, extracted knowledge, and notes. *See [The Inspector](#the-inspector) below, and [The Knowledge Graph](../5-knowledge-graph/1-the-knowledge-graph.md).*
...
- **Import** — the add menu: **Link Files…** (⌘I), **Copy Files…** (⌥⌘I), and **Move Files…** (⇧⌘I). *See [Managing Your Library](3-managing-your-library.md).*
- **Layout picker** — switches the Library Browser between Icons, List, Columns, Canvas, and Space (⌘1–⌘5).
- **Run workflow** — runs a workflow on the current selection (⇧⌘R). *See [Workflows](../3-transcribing-and-workflows/1-workflows.md).*
- **Search field** — full-text and semantic search; results render into the Library Browser. *See [Searching](../6-search/1-searching.md).*
...
- **Search** — a search field and your saved searches; results render into the same document layouts the Library uses, so you can look at a result set as icons, a list, or a table. *See [Searching](../6-search/1-searching.md).*
- **Chat** — ask questions about your documents in a conversation grounded in library retrieval.
- **Workflows** — build and run the step-by-step processing pipelines. *See [Workflows](../3-transcribing-and-workflows/1-workflows.md).*
...
- **Knowledge Graph** — browse the entities and claims extracted across the library. *See [The Knowledge Graph](../5-knowledge-graph/1-the-knowledge-graph.md).*
...
The window remembers which arrangement you last chose. Editing tools — non-destructive image and page adjustments — live on the Reader's own toolbar. *See [Reading, Transcribing, and Annotating](../4-reading/1-reading-transcribing-and-annotating.md).*
...
b. **Artifacts** — the outputs that workflows have produced for this document, each with its provenance. *See [Workflows](../3-transcribing-and-workflows/1-workflows.md).*
...
d. **Notes** — your annotations and free-text research notes, kept separate from the source text; its facets cover notes, annotations, and interpretation. *See [Reading, Transcribing, and Annotating](../4-reading/1-reading-transcribing-and-annotating.md).*

### Settings

Settings is a separate window (⌘,), where you configure Fichero — most importantly, the AI providers and models it can use. You add the providers you want and pick which models each workflow step should use. Because Fichero is model-agnostic, the model picker in a workflow shows only the models you have set up in Settings; if a picker looks empty, open Settings and add a provider first. *See [AI and Privacy](../3-transcribing-and-workflows/2-ai-and-privacy.md) for what stays on your Mac and what leaves it.*

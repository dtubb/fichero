...
Internally, the import service sends the chosen mode to the backend ingest endpoints. Imported files can also request text extraction and automatic embedding as part of ingest.
...
- Fichero shows import progress and surfaces readable backend errors when an import fails

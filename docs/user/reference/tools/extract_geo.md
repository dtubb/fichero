...
Extract place names from text and geocode them to lat/lon
...
| --- | --- | --- | --- |
| Text (`text`) | `text` | yes | Text to extract place names from |
...
| Port | Type | Required | What it is |
| --- | --- | --- | --- |
| Geo Points (`geo`) | `json` | — | Geocoded place points (lat/lon) |
...
| `online_geocoding` | boolean | no | Fall through to Nominatim for places the offline gazetteer misses. |

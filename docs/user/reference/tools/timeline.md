...
| `date_format` | string | iso | Date format. One of: iso, natural, year_only. |
| `match_mode` | string | prefer | Match mode. One of: prefer, strict, inform. |
| `max_events` | integer | 50 | Max events. |
...
```text
Create a chronological timeline of events from the provided text.

Instructions:
- Identify all datable events, actions, or occurrences
- Order them chronologically
- Maximum 50 events
- Use ISO format (YYYY-MM-DD) when possible. Use YYYY-MM or YYYY if day/month unknown.

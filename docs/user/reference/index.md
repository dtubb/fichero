...
- [Convert to HTML](workflows/convert-to-html.md) — Convert each image/page to semantic HTML using vision — preserving structure and, optionally, inline styles. Feeds the HTML/WebKit representation in the document viewer. The model alias and style options are editable on the Convert node.
- [Convert to Markdown](workflows/convert-to-markdown.md) — Convert each image/page to clean Markdown using vision — preserving headings, lists, and layout where possible. Feeds the Markdown representation in the document viewer. The model alias and layout options are editable on the Convert node.
- [Convert to SVG](workflows/convert-to-svg.md) — Recreate each image/page as SVG using vision — a vector reconstruction of the document's layout and text. Feeds the SVG (WebKit) representation in the document viewer. The model alias is editable on the Convert node.

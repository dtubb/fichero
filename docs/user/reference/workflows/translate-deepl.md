...
Transcribe each file, then translate the extracted text through DeepL (default Dutch to English). Edit source/target language on the Translate node as needed.

| | |
| --- | --- |
| Folder | /Translate |
| Steps | 3 |
| Tags | preset, translate, deepl, text, language |
...
Tool: [Translate](../tools/translate.md) — Translate text from source_lang to target_lang.
...
```text
Translate source text faithfully.
```

...
| **Visual / A11y** (Apple tools) | a surface has UI to verify | `performAccessibilityAudit()` passes + `.xctestplan` captures a screenshot (no pixel-diff) | in the Click-around leg below (`fichero/Tests/UI/**`) |
...
8. **Document each audience — by its owner.** Fill the spec's **Documentation matrix**. The agent
   ...
   **reference** (`docs/reference_manual/…`), and the MCP tool description / CLI `--help` where the feature
   ...
   produce the accurate raw material the maintainer needs (the `.xctestplan`/`RenderPreview`
   screenshot capture doubles as the doc screenshot, and the behavior list/a11y ids are current). A

...
**doc screenshot** the DOC-TEMPLATE requires for the user manual (Xcode `RenderPreview` is the
equivalent capture when you're driving the surface in Xcode). Capture the surface once, at its
seeded/fixture state; copy the committed reference into **`docs/assets/<milestone>/`** (subfolder
named for the spec/milestone) and both the Test matrix (this leg) and the Documentation matrix
...
1. **Scaffold + survey prior art.** `cp docs/contributor_manual/specs/_TEMPLATE.md
   docs/contributor_manual/specs/<surface>.md`. Fill Intent, then the **Prior art / best practices**
...
8. **Document each audience — by its owner.** Fill the spec's **Documentation matrix**. The agent
   ...
   **reference** (`docs/reference_manual/…`), and the MCP tool description / CLI `--help` where the feature
   ...
   the doc screenshot — one capture, two uses — and the behavior list/a11y ids are current). A
   feature isn't done until the reader who never opens the code can use it — and the facts that
   reader will rely on are pinned by tests, not by prose that can drift.

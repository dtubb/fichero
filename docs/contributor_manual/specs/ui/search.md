...
22 behaviors: **8 OK, 8 PARTIAL, 6 GAP.**
...
## Fold plan for the 12 + 1 issues (Pass 2 — plan only, NOT executed this pass)

**Move onto #319 (search), each now backing a named behavior above:**
...
- #4118 → `search.four-leg-response` (OK half) + `search.images-artifacts-as-first-class-
  results` (PARTIAL half)
- #4119 → `search.real-relevance-signal-exists`
- #4236 → **live bug, needs diagnosis before a fix**, not yet cited by a specific behavior
  above — a Pass 2 executor should reproduce against current HEAD (scope vs. case vs.
  page-child-not-indexed, per the issue's own three candidate causes) before writing a behavior
  line, rather than guessing which cause applies. Recommend adding
  `search.page-child-text-searchable` or similar once the cause is confirmed.
- #4395 → `search.embedding-failure-honesty`
- #4604 → `search.native-toolbar-field`, `.query-syntax-prefixes`, `.query-expansion-related-
  ...
  restored` — the umbrella EPIC, cited from every behavior descending from it.
- #4606 → `search.find-related-documents`

**Redirect elsewhere (not moved):**
- #3309 → `kg/historical-text-normalization.md` (see above).

**Net effect if Pass 2 executes as planned:** all three legacy milestones (#17, #186, #129)
would need a fresh open-issue count pulled at execution time to confirm they reach zero — this
pass did not verify whether #17/#186/#129 hold ONLY these 12 issues or others beyond what was
read (the fetch was scoped to milestone number, so the 12 should be complete, but a Pass 2
executor should re-run the milestone-number query fresh rather than assume no drift since this
read). #4236 is the one item genuinely blocked on diagnosis before a citation, not on triage or
redirect.

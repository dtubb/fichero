...
- `nodeconfig.roundtrip.open-is-read-only` — **[BROKEN-partial]** opening and closing a popover without touching anything should leave `node` byte-identical (no autosave fires). FIXED so far: the prompt half of F4 — Transcribe/Describe no longer write the default prompt on open (via `NodePromptEditor`, no autosave); AND Transcribe's legacy-`language` normalise-on-open no longer writes config — the load raises an `isLoadingConfig` guard so the seeding pickers' `onChange` don't count as edits (2026-09-16). Both pinned by `WorkflowReadOnlySavePolicyTests`. RESIDUAL (being addressed separately): Search still rewrites `search_id`/`query` on open. Normalisation of legacy values is allowed only as an explicit, visible migration — not a side effect of looking.

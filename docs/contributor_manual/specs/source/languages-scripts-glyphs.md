...
overridden: app default, library, folder, group of pages, page, region, line, word,
...
Declared signs live in a **sign list** owned by a library. A reading's text can mix ordinary
...
The font a reading needs is recorded with it. A library can carry its own fonts for its
...
palette of the library's signs and of a script's characters, searchable by name and by
...
- `source.sign.library-list` — declared signs live in a library's sign list, which can be
  exported and shared.
...
Fonts and input
- `source.font.recorded` — a reading records the font it needs; a library can carry fonts.
- `source.font.missing-is-said` — a missing font is reported; no unexplained empty boxes.
- `source.input.palette` — a searchable palette of a script's characters and the library's
...
source to use beside Glottolog; does Fichero ship fonts, or only let a library add them.

...
- `kg.tables.crud.audited` — **[PARTIAL]** every create/edit/delete is ONE typed, audited
  backend action, reversible via the mutation log (one-audited-action-layer, a standing HARD
  rule). The table's OWN create/edit/delete/curate paths ARE compliant (sections B/C above).
  The rule itself — as a cross-cutting, testable claim spanning the whole KG/entity surface,

...
- The current bundle assumes a nested macOS backend app (`docs/contributor_manual/bundling-backend.md`).
...
| `lancedb` + `pylance` + `pyarrow` | vector store/search | native macOS arm64 wheels, no iOS wheel seen | RED | Heavy native data stack; largest remaining bundle cost on Mac per `docs/contributor_manual/release/backend-python-dependencies.md`. |

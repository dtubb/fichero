...
- **Three distinct surfaces confirmed in code** (do not conflate): Preview/source-viewing (`Views/Preview/ImageViewer/`, `QuickLookPreviewViews.swift`, `MediaStreamPreview.swift`), Reader (`Views/Library/Reading/` — `ImmersiveReaderView`, `DocumentTextReader`, `ReaderTabBar`, `PDFPageView`), Inspector/editing (`Views/Library/Inspector/` — presenter, tabs, panes — and `Views/Library/DocumentInspector/` — document-scoped tabs, 24+ files, two >1,100-line files per #3439).

...
**Swift UI:**
- Owner side: `InviteAccountSection` (`Views/Settings/Sharing/InviteAccountSection.swift`, #3157) — mint form, pending-invite list with Revoke, and `InviteLinkSheet` (`:133-206`) showing QR + selectable link + **Copy Link**. **No `ShareLink`** — there is no "send via Messages/Mail" affordance here, only copy (`:174-186`).

...
`docs/user/` and `docs/contributor/` cite entries by key; a guardrail proves every citation
...
- `cite.docs-resolve` [MISSING, hard] — every citation key used in `docs/user/**` and
  `docs/contributor/**` resolves to a `.bib` entry; an orphan citation fails the gate

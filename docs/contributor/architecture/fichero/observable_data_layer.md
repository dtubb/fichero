...
The `.ficheroClaimUpdated` / `.ficheroClaimDeleted` / `.ficheroEntitySearchRequested` posts (audit S2; in `ClaimSummaryCardView`, `OntologyBrowser`, `ClaimReviewQueueSheet`, `ContradictionTriageSheet`, `Inspector/Knowledge/*`) are the **interim** cross-view mutation bus. They are retired **per store, as that store lands**, not in a big-bang:

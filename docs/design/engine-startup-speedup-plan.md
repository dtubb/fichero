...
import fichero_server.api.main, .pyc absent (cold). 4.30s  (one-time bytecode compile)
import fichero_server.api.main, .pyc present (warm) 2.77s  → 2.47s on repeat runs
```
...
Reproduce: `PYTHONPATH=<worktree>/fichero-server/src /Users/danieltubb/code/fichero/.venv/bin/python -X importtime -c "import fichero_server.api.main" 2> /tmp/importtime.log`

## 2. Where the ~1.82s of `import fichero_server.api.main` cumulative time actually goes
...
|---|---:|---:|---|
| **`fichero_server.api.routes.*` (94 modules)** | **801** | **44%** | One monolithic `from fichero_server.api.routes import (...)` block, main.py:1286–1370. Imported **unconditionally**, before any feature-tier check. |
...
| `fichero_server.models` + `fichero.knowledge_models`/`knowledge.knowledge_models` | ~80 | 4% | Domain Pydantic model modules, imported by nearly every route module. |
...
2. **The `fichero_server.api.routes.*` monolithic import block (801ms, 44% of
   ...
   built) instead of one static `from fichero_server.api.routes import (...)` block.
...
| **3** | Make route-module import itself tier-aware (import only the modules whose spec is enabled for the resolved tier, inside `get_route_specs_for_tier`/`register_tiered_routes`, not via the static top-of-file `from fichero_server.api.routes import (...)`) | Proportional to modules excluded by Stage 2 — e.g. excluding ~15 dev-only modules (`kg_sparql`, `kg_curation_rules`, `kg_claim_analysis`, `search_explain`, `iiif`, `triggers`, `multilingual`, `citation_usages`, `canvas`, `local_models`, `mcp_servers`, `orchestration`, ...) would save roughly 100–150ms at `release` tier, based on their measured per-module self-times | Medium — must preserve OpenAPI contract stability for the enabled tier; contract tests (`fichero-server/tests/contracts/openapi.json`) must still pass for whichever tier CI runs | `fichero/api/main.py` |
...
```
PYTHONPATH=<worktree>/fichero-server/src /Users/danieltubb/code/fichero/.venv/bin/python -X importtime -c "import fichero_server.api.main" 2>&1 | tail -1
```
(reports the cumulative import time for `fichero_server.api.main`; compare the
cumulative-µs figure before/after each stage, and re-run
`time python -c "import fichero_server.api.main"` three times warm to sanity-check
wall-clock.)

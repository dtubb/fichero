...
class places_json(BaseModel):
  places: str = Field(description="Name of places.")
...
def main(source_file: str,
  ...
  """
  Extract PLACES from a source file, and return it as a JSON.
  ...
  output_parser = JsonOutputParser(pydantic_object=places_json)

  prompt_template = """Extract ALL NAMES of PLACES mentioned in the SOURCE. SOURCE. DO NOT ASSUME. SAY NOTHING ELSE.
...
{source}"""
  ...
  if llm_model == "gpt-4" or llm_model == "gpt-3.5-turbo-instruct":
    ...
    prompt = PromptTemplate(
      template="""Extract ALL NAMES of PLACES mentioned in the SOURCE. DO NOT ASSUME. SAY NOTHING ELSE.
...
{source}""",
  ...
  extracted_places = chain.invoke({"source": f"{source}"})
  pprint.pprint(extracted_places)

  ## Extract Names
  print("\nRunning QA on Extracted Names…\n")
  prompt_template = """Undertake quality assurance on NAMES of PLACES extracted from SOURCE. Flag all errors, e.g. missing place names, entities mistakenly listed as places that are not, places with multiple spellings (recommend most likely spelling), etc. BE HARSH, BUT DO NOT MAKE ASSUMPTIONS. Return a JSON of errors to fix. USE SPANISH. SAY NOTHING ELSE. 

JSON of EXTRACTED PLACES:
{extracted_places}
...
""" 
  ...
  qa_recomendations = chain.invoke({"source": f"{source}",
                      "extracted_places": f"{extracted_places}"})
                      
  pprint.pprint(qa_recomendations)

  print("\nCleaning up Extracted Places based on QA…\n")
  prompt_template = """Implement recommendations to fix PLACES NAME mentioned in SOURCE. Also, change to Normal Caps. DO NOT USE ALL CAPS. Return a JSON of ONLY FIXED NAMES. SAY NOTHING ELSE.
...
""" 
  ...
  output_parser = JsonOutputParser(pydantic_object=places_json)
  chain = prompt | llm | output_parser
  fixed_places = chain.invoke({"source": f"{source}",
                      "extracted_places": f"{extracted_places}",
                      "qa_recomendations": f"{qa_recomendations}"})  
   
  print("\nFixed Places:")
  pprint.pprint(fixed_places)

  with open(output_file, 'w') as f:
    json.dump(fixed_places, f)

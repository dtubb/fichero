#!/usr/bin/env python3
"""
Script to add IIIF import functionality to the library commands
"""
...
# Add new imports for IIIF functionality
...
# Add IIIF import command to the CLI commands
iiif_command = '''
...
# Find where to insert the command (after the structure command)
structure_command_end = 'def structure(collection_id: str, depth: int = 3):'
insertion_point = content.find(structure_command_end)
if insertion_point != -1:
    # Find the end of the structure function
    structure_end = content.find('@library_app.command("process")', insertion_point)
    if structure_end != -1:
        content = content[:structure_end] + iif_command + '\n\n' + content[structure_end:]

# Add the IIIF import implementation methods
iiiif_methods = '''
async def _iiif_import(manifest_path: Path, collection_name: str, output_dir: Optional[Path], batch_size: int, timeout: int):
    """Import images from IIIF manifests or URLs"""
    try:
        ...
        collection_id = await _download_iiif_collection(manifest_uris, collection_name, output_dir, batch_size, timeout)
...
async def _download_iiif_collection(manifest_uris: List[str], collection_name: str, output_dir: Path, batch_size: int, timeout: int) -> Optional[str]:
        ...
        with Progress() as progress:
            ...
            for manifest in manifests:
                if "sequences" in manifest and manifest["sequences"]:
                    ...
                    srsly.write_json(manifest_folder / "metadata.json", metadata)
                    ...
                    for image in images:
                        if "images" in image and image["images"]:
                            ...
                            if "@id" in image_resource:
                                ...
                                image_name = _generate_image_name(image_uri)
                                image_path = manifest_folder / image_name
                                
                                if not image_path.exists():
                                    image_tasks.append(_download_image(client, image_uri, image_path, image_task, progress))
                                else:
...
async def _download_image(client: httpx.AsyncClient, image_uri: str, output_path: Path, task, progress: Progress):
...
def _generate_image_name(image_uri: str) -> str:
...
# Add the methods at the end of the class, before the last method
class_end = '    async def _get_collection_by_id(self, collection_id: str):'
insertion_point = content.find(class_end)
if insertion_point != -1:
    content = content[:insertion_point] + iiif_methods + '\n\n    ' + content[insertion_point:]

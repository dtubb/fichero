...
# Engine for the Dev Local scheme (Debug config never spawns one). UDS only;
...
- **Two pytest runs in ONE worktree deadlock each other.** The suite opens DuckDB files under the worktree, so a second run contends on the same locks: both stall, neither errors, and the logs simply stop — one sat at 16% for fifteen minutes with no output and no failure (2026-09-26). This bites hardest with parallel lanes, where a worker runs its own targeted tests in the same tree the manager is gating. Before starting a full gate, check: `pgrep -fl pytest`. One run per worktree; a lane's targeted run and the manager's gate are not compatible, and the gate is the one that should yield, since it can be re-run cheaply.

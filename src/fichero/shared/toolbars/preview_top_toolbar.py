...
class PreviewTopToolbar(TopToolbar):
    ...
    def _create_toolbar(self):
        ...
        try:
            ...
            # Preview toolbar approach:
            # Mobile: back button only (no title in toolbar)
            # Desktop: no back button (no title in toolbar)
            # Both: content title will be in BaseView content area
            
            if self.is_mobile:
                # Mobile: back button + label showing where we came from (like collection name)
                ...
                # Add label showing the collection/folder we came from
                self.title_label = self.add_title_left(
                    "Collection",  # Default, will be updated by callback registration
                    margin_left=10,
                    on_click=self._on_title_pressed
                )
            else:
                # Desktop: no back button needed
                self.back_button = None
                self.title_label = None  # No title in toolbar

"""
Processing Window for Fichero - Thin Wrapper

Lightweight processing window that delegates all business logic to director.py.
...
from fichero.utils.path_icons import render_path_with_icons, get_folder_icon_path
...
class FicheroProcessingWindow(toga.Window):
    ...
    def _draw_ready_background_and_text(self, canvas=None, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error drawing ready background and text: {e}")
            # Don't re-raise to prevent repeated error loops
    ...
    def _draw_plan_description_content(self, canvas):
        ...
        try:
            ...
            logger.warning(f"Could not create custom fonts for plan description, using defaults: {e}")
        ...
        try:
            ...
            logger.warning(f"Could not measure ready text, using fallback positioning: {e}")
            ready_x = 20  # Fallback left margin
            ready_y = current_y + (regular_font.size * line_height_multiplier * 0.8)
    ...
    def _draw_content_text(self, canvas=None, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error drawing content text: {e}")
            # Don't re-raise to prevent repeated error loops
    ...
    def _draw_description_content(self, canvas):
        ...
        # Use safer font initialization to avoid version compatibility issues
        try:
            regular_font = toga.Font(family=toga.fonts.SYSTEM, size=10, weight="light")
            bold_font = toga.Font(family=toga.fonts.SYSTEM, size=10, weight="bold")
        except Exception as e:
        ...
        for element in elements:
            ...
            for word_idx, word in enumerate(words):
                ...
                try:
                    ...
                    logger.warning(f"Could not measure word width, using estimate: {e}")
                    # Fallback to character-based width estimation
                    word_width = len(test_word) * 6  # Rough estimate
                ...
                if current_line_width + word_width <= max_width:
                    ...
                    try:
                        ...
                        logger.warning(f"Could not measure word width for new line, using estimate: {e}")
                        word_width = len(word) * 6  # Rough estimate
    ...
    def _create_task_display(self):
        ...
        try:
            ...
            self.task_display = GUITaskDisplay(
                task_monitor, 
                filter_document_id=self._document_id
            )
            ...
            logger.info(f"Created task display for processing window: {self._document_id}")

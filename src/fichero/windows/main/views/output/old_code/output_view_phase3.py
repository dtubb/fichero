...
class OutputView(BaseView, ViewCommandMixin):
    ...
    def _setup_toolbars(self):
        ...
        try:
            ...
            # Add Export button to top toolbar (right side, before Edit button)
            # This exports the current step's outputs to a user-selected location
            ...
            self.export_btn.enabled = False  # Disabled until outputs are loaded
    ...
    async def _on_export_pressed(self, widget):
        """Handle Export button press - export current step's outputs"""
        try:
            # Check if we have processing steps loaded
            if not self.processing_steps or self.current_step_index >= len(self.processing_steps):
                logger.warning("No processing steps available to export")
                await self.app.main_window.info_dialog(
                    _("No Outputs"),
                    _("No processing outputs available to export.")
                )
                return
            ...
            # Get output files
            output_files = current_step.files

            if not output_files:
                logger.warning(f"No output files found for step: {tool_name}")
                await self.app.main_window.info_dialog(
                    _("No Files"),
                    _("No output files found for this step.")
                )
                return

            logger.info(f"Exporting {len(output_files)} files from step: {tool_name}")

            # Show folder picker dialog
            try:
                export_folder = await self.app.main_window.select_folder_dialog(
                    title=_("Select Export Destination"),
                ...
                # Create subfolder named after the tool
                tool_folder = export_path / tool_name
                ...
                for file_path in output_files:
                    if Path(file_path).exists():
                        dest_file = tool_folder / Path(file_path).name
                ...
                await self.app.main_window.info_dialog(
                    _("Export Complete"),
                    _("Exported {count} files to:\n{path}").format(
                        count=copied_count,
                        path=tool_folder
                    )
                )

                logger.info(f"Successfully exported {copied_count} files to {tool_folder}")

            except Exception as dialog_error:
                logger.error(f"Error during export dialog or file copy: {dialog_error}")
                await self.app.main_window.error_dialog(
                    _("Export Failed"),
                    _("Failed to export files: {error}").format(error=str(dialog_error))
                )

        except Exception as e:
            logger.error(f"Failed to export outputs: {e}", exc_info=True)
            await self.app.main_window.error_dialog(
                _("Export Error"),
                _("An error occurred during export: {error}").format(error=str(e))
            )
    ...
    def load_output(self, file_path: Path = None, source_files: List[Path] = None, source_index: int = 0,
        ...
        try:
            ...
            if item_id:
                ...
                # Note: Don't update file nav buttons here - let _load_from_library() handle it
                # after determining if this is a folder or file (avoids premature updates)
    ...
    async def _load_from_library(self, item_id: str):
        ...
        try:
            ...
            # Debug: Log what we got from library manager
            if output_data:
                processing_steps_count = len(output_data.get('processing_steps', []))
                director_status = output_data.get('director_status', 'unknown')
                logger.info(f"📦 Library returned output_data for item {item_id}: "
                          f"processing_steps={processing_steps_count}, "
                          f"director_status={director_status}")
                if processing_steps_count > 0:
                    for idx, step in enumerate(output_data.get('processing_steps', [])):
                        logger.debug(f"  Step {idx}: {step.step_name if hasattr(step, 'step_name') else 'unknown'}")
            else:
                logger.info(f"📦 Library returned None for item {item_id}")
            ...
            is_folder = output_data.get('is_folder', False)

            # If this is a folder, organize steps by step_name for filtering
            if is_folder and processing_steps:
                logger.info(f"📁 Folder detected with {len(processing_steps)} steps - organizing by step name")

                # Organize steps by step_name
                steps_by_name = {}
                for step in processing_steps:
                    ...
                    steps_by_name[step_name].append(step)

                # Get unique step names in order (preserving workflow order)
                unique_step_names = []
                seen_names = set()
                for step in processing_steps:
                    step_name = step.step_name if hasattr(step, 'step_name') else 'Unknown'
                    if step_name not in seen_names:
                        unique_step_names.append(step_name)
                        seen_names.add(step_name)

                logger.info(f"Found {len(unique_step_names)} unique steps: {unique_step_names}")

                # Store organized steps and output_data for folder view
                self.folder_steps_by_name = steps_by_name
                self.folder_step_names = unique_step_names
                self.folder_output_data = output_data  # Store for refresh
                self.processing_steps = []  # Will be populated dynamically per step
                self.current_step_index = 0  # Start with first step
                self.is_folder_view = True

                # Update step selector with folder step names
                self._update_folder_step_selector()

                # Show folder view with first step selected
                self._show_folder_view(output_data, step_index=0)
                return
            ...
            # Restore saved step index if available (preserves step across file navigation)
            saved_step_index = 0
            if hasattr(self, 'viewer_state') and self.viewer_state and 'step_index' in self.viewer_state:
                saved_step_index = self.viewer_state.get('step_index', 0)
                # Clamp to valid range for new file's steps
                saved_step_index = max(0, min(saved_step_index, len(self.processing_steps) - 1))
                logger.debug(f"Restoring saved step index: {saved_step_index}")

            self.current_step_index = saved_step_index
            self.left_step_index = max(0, saved_step_index - 1)
            self.right_step_index = saved_step_index
            ...
            # Enable export button now that outputs are loaded
            if hasattr(self, 'export_btn'):
                self.export_btn.enabled = len(self.processing_steps) > 0
    ...
    def _update_file_navigation_buttons(self):
        ...
        try:
            # Check if we're in folder view (no source files) or file view
            if not self.source_files:
                # Folder view - no file navigation
                if hasattr(self.top_toolbar, 'title_label') and self.top_toolbar.title_label:
                    self.top_toolbar.title_label.text = "Folder View"
                logger.debug("Folder view - no file navigation")
                return

            # File view - show file navigation
            has_prev = self.current_source_index > 0
            has_next = self.current_source_index < len(self.source_files) - 1
            ...
            logger.debug(f"File view navigation: prev={has_prev}, next={has_next}, file={self.current_source_index + 1}/{len(self.source_files)}")
    ...
    async def _save_viewer_state(self):
        """Save current viewer state (zoom, rotation, scroll position, current step)"""
        try:
            if hasattr(self, 'current_webview') and self.current_webview:
                ...
                self.viewer_state = {
                    ...
                    'step_index': self.current_step_index  # Preserve current step across file navigation
                }
    ...
    def _show_single_view(self):
        ...
        current_step = self.processing_steps[self.current_step_index]
        logger.debug(f"_show_single_view: step={current_step.tool_name if hasattr(current_step, 'tool_name') else 'unknown'}, files={current_step.files if hasattr(current_step, 'files') else 'none'}")
        if current_step.files:
            ...
            logger.debug(f"_show_single_view: displaying file={current_file}")
    ...
    def _show_folder_view(self, output_data: dict, step_index: int = 0):
        """Show web-based folder view with thumbnails filtered by selected step

        Args:
            output_data: Dictionary containing folder output data
            step_index: Index of the step to display (filters to show only files from this step)
        """
        try:
            folder_name = output_data.get('filename', 'Folder')
            output_root = output_data.get('output_root')

            # Get the step name for this index
            if not hasattr(self, 'folder_step_names') or step_index >= len(self.folder_step_names):
                logger.error(f"Invalid step_index {step_index} or folder_step_names not set")
                self._show_error_message("Invalid step selection")
                return

            current_step_name = self.folder_step_names[step_index]
            steps_at_current_level = self.folder_steps_by_name.get(current_step_name, [])

            logger.info(f"Creating folder view for '{folder_name}' at step '{current_step_name}' ({len(steps_at_current_level)} files)")

            # Clear file navigation state (folders don't have prev/next file navigation)
            self.source_files = []
            ...
            # Group files by filename for current step only
            files_by_name = {}
            for step in steps_at_current_level:
            ...
            sorted_files = sorted(files_by_name.keys())

            # Check for groups manifest (for visual grouping)
            groups_data = None
            if output_root:
                from pathlib import Path
                output_path = Path(output_root)
                groups_manifest = output_path / "groups_manifest.jsonl"
                if groups_manifest.exists():
                    try:
                        import json
                        with open(groups_manifest, 'r') as f:
                            groups_data = json.load(f)
                            logger.info(f"Found groups manifest with {len(groups_data.get('analysis', {}).get('groups', []))} groups")
                    except Exception as e:
                        logger.warning(f"Could not load groups manifest: {e}")

            # Create HTML for folder view
            html_content = self._generate_folder_view_html(
            ...
            # Store file mapping for click handling
            # We need to map filename → child item_id for navigation
            self.folder_file_mapping = {}
            for idx, filename in enumerate(sorted_files):
                self.folder_file_mapping[idx] = filename

            # Initialize empty mapping (will be populated by background task)
            self.folder_child_item_mapping = {}

            # Get child items to build filename → item_id mapping in background
            # This allows us to navigate from clicked filename to individual item view
            if hasattr(self, 'current_item_id') and self.current_item_id:
                async def build_child_mapping():
                    try:
                        # Get all items in the folder's collection
                        folder_item = await self.library_manager.get_item(self.current_item_id)
                        if folder_item:
                            collection_items = await self.library_manager.get_collection_items(folder_item.collection_id)

                            # Build mapping from source filename → item_id for children
                            mapping = {}
                            for child_item in collection_items:
                                if child_item.parent_id == self.current_item_id:
                                    # Get the source filename
                                    if hasattr(child_item, 'source_path') and child_item.source_path:
                                        source_path = Path(child_item.source_path)
                                        child_filename = source_path.name
                                        mapping[child_filename] = child_item.id
                                        logger.debug(f"Mapped {child_filename} → {child_item.id}")

                            self.folder_child_item_mapping = mapping
                            logger.info(f"Built mapping for {len(self.folder_child_item_mapping)} child items")
                    except Exception as e:
                        logger.error(f"Failed to build child item mapping: {e}")
                        import traceback
                        logger.error(traceback.format_exc())

                # Schedule the background task
                self.app.add_background_task(build_child_mapping)

            # Create WebView to display folder with click handler
            webview = toga.WebView(
                style=Pack(
                    flex=1,
                    margin=5
                ),
                on_webview_load=self._on_folder_thumbnail_clicked
            )
            webview.set_content(root_url="", content=html_content)

            self.content_area.add(webview)

            # Update file navigation buttons to reflect folder view (no file navigation)
            self._update_file_navigation_buttons()

            logger.info("✅ Folder view displayed successfully")

        except Exception as e:
            logger.error(f"Error showing folder view: {e}")
            import traceback
            logger.error(traceback.format_exc())
            self._show_error_message(f"Failed to display folder view: {e}")

    def _on_folder_thumbnail_clicked(self, widget, **kwargs):
        """Handle thumbnail clicks in folder view by intercepting URL navigation

        When user clicks a thumbnail, the WebView tries to navigate to:
        fichero://open-file?index=X&filename=Y

        This method intercepts that navigation and loads the file instead.

        Args:
            widget: The WebView widget that triggered the event
            **kwargs: Additional event parameters
        """
        try:
            url = widget.url
            if not url or not url.startswith('fichero://open-file?'):
                return

            logger.info(f"🖱️ Folder thumbnail clicked: {url}")
            ...
            if 'index' not in params:
                logger.warning("No index parameter in click URL")
                return

            file_index = int(params['index'][0])
            filename = self.folder_file_mapping.get(file_index)

            if not filename:
                logger.error(f"No file found for index {file_index}")
                return

            logger.info(f"Thumbnail clicked: {filename} (index {file_index})")

            # Look up the child item_id for this filename
            if not hasattr(self, 'folder_child_item_mapping'):
                logger.error("No folder_child_item_mapping available")
                return

            child_item_id = self.folder_child_item_mapping.get(filename)
            if not child_item_id:
                logger.error(f"No child item_id found for filename: {filename}")
                return

            logger.info(f"Navigating to child item: {child_item_id}")

            # Navigate to the child item's individual view at the current step
            # Use async task to load the item
            async def load_child_item():
                try:
                    await self._load_from_library(child_item_id)
                    # After loading, jump to the step that was selected in folder view
                    if hasattr(self, 'current_step_index') and self.current_step_index > 0:
                        # current_step_index is from folder view
                        # We need to navigate to that step in the individual file view
                        if self.current_step_index < len(self.processing_steps):
                            self._show_step(self.current_step_index)
                except Exception as e:
                    logger.error(f"Failed to load child item {child_item_id}: {e}")
                    import traceback
                    logger.error(traceback.format_exc())

            # Schedule the async load
            self.app.add_background_task(load_child_item)

            # Prevent actual navigation
            # Note: Setting widget.url = None may not work on all platforms
            # The key is that we've already initiated the navigation via background task
    ...
    def _generate_thumbnail_data(self, file_path: Path) -> str:
        """Generate HTML for a single thumbnail - either base64 image or icon

        Args:
            file_path: Path to the file to generate thumbnail for

        Returns:
            HTML string with either <img> tag or icon div
        """
        import base64

        ext = file_path.suffix.lower()

        # For images, encode as base64 and embed
        if ext in {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp'}:
            try:
                with open(file_path, 'rb') as f:
                    image_data = base64.b64encode(f.read()).decode('utf-8')
                mime_type = 'image/jpeg' if ext in {'.jpg', '.jpeg'} else f'image/{ext[1:]}'
                return f'<img src="data:{mime_type};base64,{image_data}" alt="Thumbnail" />'
            except Exception as e:
                logger.error(f"Failed to encode image {file_path}: {e}")
                return '<div class="thumbnail-icon">🖼️</div>'

        # For text files, show text icon
    ...
    def _generate_folder_view_html(self, folder_name: str, sorted_files: list,
                                   files_by_name: dict, groups_data: dict = None) -> str:
        """Generate HTML content for folder view - clean grid of thumbnails on black background

        Args:
            folder_name: Name of the folder
            sorted_files: List of filenames sorted alphabetically
            files_by_name: Dictionary mapping filenames to their processing steps with file_path
            groups_data: Optional groups manifest data for visual grouping (unused for now)

        Returns:
            HTML string for the folder view
        """
        # Generate thumbnail data with base64-encoded images and click handlers
        thumbnails_html = ""
        for idx, filename in enumerate(sorted_files):
            ...
            # Get the file path for this step (first entry in file_data)
            file_path = Path(file_data[0]['file_path'])

            # Generate thumbnail preview based on file type
            thumbnail_data = self._generate_thumbnail_data(file_path)

            # Create click URL - encode filename to handle special characters
            import urllib.parse
            encoded_filename = urllib.parse.quote(filename)
            click_url = f"fichero://open-file?index={idx}&filename={encoded_filename}"

            thumbnails_html += f"""
            <a href="{click_url}" class="thumbnail" data-file-index="{idx}" data-filename="{filename}">
                {thumbnail_data}
            </a>
"""

        # Build HTML with black background and clean grid
        html = f"""
<!DOCTYPE html>
<html>
<head>
    ...
    <script>
        function clickThumbnail(filename) {{
            // Store clicked file in console for debugging
            console.log('Clicked:', filename);
            // TODO: Post message back to Python
            // For now, we'll use a different approach via Toga
        }}
    </script>
</head>
<body>
...
"""
    ...
    def _create_thumbnail_strip(self):
        """Create horizontal scrollable thumbnail strip showing all processing steps

        Returns:
            ScrollContainer with horizontal thumbnail buttons
        """
        logger.debug(f"Creating thumbnail strip with {len(self.processing_steps)} steps")
        for idx, step in enumerate(self.processing_steps):
            logger.debug(f"  Step {idx}: {step.tool_name}, files: {len(step.files) if step.files else 0}")

        # Container for the thumbnail strip
        strip_container = toga.Box(
            style=Pack(
                direction=COLUMN,
                margin_top=10,
                height=130  # Fixed height for thumbnail strip
            )
        )
        ...
        # Create thumbnail for each step
        for idx, step in enumerate(self.processing_steps):
            is_current = (idx == self.current_step_index)

            # Determine status indicator
            has_files = step.files and len(step.files) > 0
            status_icon = "✓" if has_files else "⚠"

            # Step name (truncate if too long)
            step_name_short = step.tool_name[:8] if len(step.tool_name) > 8 else step.tool_name

            # Create a container for this step's thumbnail
            thumb_container = toga.Box(
                style=Pack(
                    direction=COLUMN,
                    margin=5,
                    width=90,
                    background_color='#d0d0d0' if not is_current else '#90b0ff'
                )
            )

            # Clickable button for this step
            step_button = toga.Button(
                f"{idx + 1}",
                on_press=lambda widget, step_idx=idx: self._on_thumbnail_click(step_idx),
                style=Pack(
                    margin=2,
                    font_size=14,
            ...
            thumb_container.add(step_button)
            ...
            thumb_container.add(name_label)
            ...
            thumb_container.add(status_label)

            scroll_box.add(thumb_container)

        # Wrap in ScrollContainer for horizontal scrolling
        ...
        strip_container.add(scroll_container)
        return strip_container

    def _on_thumbnail_click(self, step_idx: int):
        """Handle click on thumbnail to navigate to that step

        Args:
            step_idx: Index of the step to navigate to
        """
        if 0 <= step_idx < len(self.processing_steps):
            self.current_step_index = step_idx
            self.current_file_index = 0
            # Update split view indices
            self.left_step_index = max(0, step_idx - 1)
            self.right_step_index = step_idx
            self._load_current_file()
            self._update_navigation()

    def _create_pane_box(self, file_path, step_name: str):
        """Create a pane box for displaying content with horizontal thumbnail strip
        ...
        """
        logger.debug(f"_create_pane_box: step_name={step_name}, file_path={file_path}, file_path_type={type(file_path)}")
        ...
        content_box.add(content)

        pane.add(content_box)

        # Add horizontal thumbnail strip if we have processing steps (even if just 1)
        # This shows "Original" when nothing has been processed, or all steps when processed
        if self.processing_steps and len(self.processing_steps) >= 1:
            thumbnail_strip = self._create_thumbnail_strip()
            pane.add(thumbnail_strip)
    ...
    def _create_file_viewer(self, file_path):
        ...
        if not file_path.exists():
            ...
            logger.error(f"Cannot display file - does not exist: {file_path}")
            return toga.MultilineTextInput(
                value=error_msg,
                readonly=True,
                style=Pack(flex=1, margin=10, font_family='monospace')
            )
    ...
    def _show_original_as_single_step(self, file_path):
        ...
        # Create a mock ToolOutput for the original file so thumbnail strip can show it
        if isinstance(file_path, str) and file_path.startswith(('http://', 'https://')):
            # URL - create a temporary path object
            mock_path = Path("/original")
        else:
            mock_path = Path(file_path) if not isinstance(file_path, Path) else file_path

        # Create a single pseudo-step representing the original file
        original_step = ToolOutput(
            tool_name="Original",
            output_folder=mock_path.parent if mock_path != Path("/original") else Path("/"),
            manifest_path=mock_path.parent / "original.jsonl" if mock_path != Path("/original") else Path("/original.jsonl")
        )
        original_step._files = [file_path]  # Set the file manually

        self.processing_steps = [original_step]
        ...
        # Hide step navigation buttons (only one step - the original)
        self.prev_step_btn.style.display = "none"
        self.next_step_btn.style.display = "none"

        logger.debug("Showing original file as single step with thumbnail strip")
    ...
    def _update_folder_step_selector(self):
        """Update the step selector dropdown for folder view with unique step names"""
        try:
            if not hasattr(self, 'step_selector') or not self.step_selector:
                return

            if not hasattr(self, 'folder_step_names') or not self.folder_step_names:
                self.step_selector.items = ["No steps loaded"]
                self.step_selector.enabled = False
                return

            # Build list of step names for folder (numbered)
            step_names = []
            for idx, step_name in enumerate(self.folder_step_names):
                formatted_name = f"{idx + 1}. {step_name}"
            ...
            if hasattr(self, 'current_step_index') and 0 <= self.current_step_index < len(step_names):
                try:
                    self.step_selector.value = self.step_selector.items[self.current_step_index]
                except Exception as e:
                    logger.debug(f"Could not set step selector value: {e}")

            logger.debug(f"Updated folder step selector with {len(step_names)} steps")

        except Exception as e:
            logger.error(f"Failed to update folder step selector: {e}")
    ...
    def _update_step_selector(self):
        ...
        try:
            ...
            # Set current selection - must be done after items are set
            # and the value must be from the items list
            if hasattr(self, 'current_step_index') and 0 <= self.current_step_index < len(step_names):
                try:
                    # Access the item directly from the items list to ensure object identity
                    self.step_selector.value = self.step_selector.items[self.current_step_index]
                except Exception as e:
                    logger.debug(f"Could not set step selector value: {e}")
    ...
    def _on_step_selected(self, widget):
        ...
        try:
            ...
            if hasattr(self, 'is_folder_view') and self.is_folder_view:
                # Folder view mode - refresh folder view with selected step
                if 0 <= step_index < len(self.folder_step_names):
                    step_name = self.folder_step_names[step_index]
                    logger.info(f"Folder step selected: {step_index + 1} ({step_name})")

                    # Update current step index
                    self.current_step_index = step_index

                    # Refresh folder view to show files at this step level
                    if hasattr(self, 'folder_output_data'):
                        self._show_folder_view(self.folder_output_data, step_index=step_index)
                return

            # File view mode - standard behavior
            if not self.processing_steps:
                return

...
class StepManager:
    ...
    def set_current_step(self, index: int) -> bool:
        ...
        self.logger.info(f"🎯 CLICK TRACE #11.3: set_current_step called with index={index}, len(steps)={len(self.steps)}")

        if 0 <= index < len(self.steps):
            self.logger.info(f"🎯 CLICK TRACE #11.4: ✅ Index valid, setting current_step_index and emitting state change")
            self.current_step_index = index
            self._emit_state_change()  # PHASE 4: emit event
            return True

        self.logger.error(f"🎯 CLICK TRACE #11.4: ❌ Index {index} out of range! Steps: {len(self.steps)}")
        return False
    ...
    def _emit_state_change(self):
        ...
        if self._emitting_state_change:
            self.logger.debug("🎯 _emit_state_change: Already emitting, skipping (recursive guard)")
            return

        self.logger.info(f"🎯 CLICK TRACE #11.5: _emit_state_change called, on_state_changed callback set? {self.on_state_changed is not None}")

        if self.on_state_changed:
            try:
                self._emitting_state_change = True
                state = self.get_state()
                self.logger.info(f"🎯 CLICK TRACE #11.6: Calling on_state_changed callback with state: step={state.step_index}, item_id={state.item_id}")
            ...
            self.logger.error(f"🎯 CLICK TRACE #11.5: ❌ NO CALLBACK SET! on_state_changed is None!")

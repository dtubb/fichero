...
NEXT_STEP_TEMPLATE = """You are expert AI prompt composer tasked with writing a prompt that responds to a question ~500 words. You are asked to write a prompt to do the following task: {question}.
...
1. Revise your previous answer using the new information.
    ...
    - Please add, if required, place holders for the variables, like using python conventions, and other things you might need to ask. 
...
Revised answer: [give your revised answer based on the previous critique and new information from the search engine if required]
...
INITIAL_ANSWER_TEMPLATE = """You are expert ai prompt writer trying write a prompt for an AI ~250 words. You are asked to write a prompt to do the following task: {question}
...
workflow.add_conditional_edges(
    'tools',
    lambda x: "exit" if len(x['intermediate_steps']) >= 5 else "continue",
    {
        "continue": 'next',
        "exit": 'finish'
    }
...
for output in chain.stream(
    {"question": """Your task is to write me a letter to a unviersity president about the need to ban cellphoens to 

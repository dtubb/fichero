...
from fichero.api.routes import kg_graph as routes
from fichero.knowledge.graph import CentralityScore


def test_centrality_maps_scores_and_filters_type(monkeypatch):
    graph = nx.MultiDiGraph()
    calls = []
    monkeypatch.setattr("fichero.kg.graph.build_full_cooccurrence", lambda db: graph)
    monkeypatch.setattr(
        "fichero.kg.graph.centrality",
...
def test_cooccurrence_sorts_neighbors_and_rejects_unknown_entity(monkeypatch):
    ...
    monkeypatch.setattr("fichero.kg.graph.build_full_cooccurrence", lambda _db: graph)
...
def test_shortest_path_reports_edge_length(monkeypatch):
    graph = nx.MultiDiGraph()
    monkeypatch.setattr("fichero.kg.graph.build_full_graph", lambda db: graph)
    monkeypatch.setattr(
        "fichero.kg.graph.shortest_path_entities",

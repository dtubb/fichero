...
async def test_translate_text_routes_to_deepl(monkeypatch):
    ...
    monkeypatch.setattr("fichero.llm._translate_with_deepl", _fake_deepl)
...
async def test_translate_tool_saves_translation_artifact(monkeypatch):
    from fichero.workflows.tools.translate import translate

    monkeypatch.setattr(
        "fichero.workflows.tools.translate.translate_text",
        AsyncMock(return_value="Hello world"),
    )
    save_artifact = AsyncMock(return_value="artifact-123")
    monkeypatch.setattr(
        "fichero.workflows.tools.translate.save_artifact",
        save_artifact,
    )
    monkeypatch.setattr(
        "fichero.workflows.tools.translate.save_to_file",

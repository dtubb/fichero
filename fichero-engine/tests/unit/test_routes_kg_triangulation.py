...
from fichero.api.routes import kg_triangulation as routes
...
def test_entity_triangulation_maps_support_records(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        "fichero.kg.triangulation.triples_for_entity",
...
def test_library_triangulation_forwards_threshold(monkeypatch):
    calls = []
    monkeypatch.setattr(
        "fichero.kg.triangulation.triangulated_facts",
...
def test_recompute_reports_updated_claim_count(monkeypatch):
    monkeypatch.setattr("fichero.kg.triangulation.persist_support_counts", lambda db: 7)

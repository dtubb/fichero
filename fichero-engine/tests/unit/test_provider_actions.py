...
import fichero.api.routes.providers  # noqa: F401
import fichero.api.routes.provider_keys  # noqa: F401
from fichero.actions.registry import ActionContext, registry
from fichero.models import ActionAudit, Model, Provider, ProviderRef, ProviderType
...
class TestProviderCreateAction:
    ...
    def test_create_secret_never_lands_in_audit(self, db, app_db, monkeypatch):
        ...
        monkeypatch.setattr(
            "fichero.api.routes.providers.set_api_key", lambda pt, k: True
        )
        result = registry.invoke(
            db,
            "provider.create",
...
class TestApiKeyActions:
    def test_set_api_key_stores_without_leaking_secret(self, db, app_db, monkeypatch):
        ...
        emitted: list[tuple] = []
        monkeypatch.setattr(
            "fichero.api.routes.provider_keys.keychain_available", lambda: True
        )
        monkeypatch.setattr(
            "fichero.api.routes.provider_keys.set_api_key",
    ...
    def test_delete_api_key(self, db, app_db, monkeypatch):
        ...
        monkeypatch.setattr(
            "fichero.api.routes.provider_keys.delete_api_key",

...
NOTE: db_manager is imported INSIDE save_artifact via `from fichero.db import
db_manager`, so the patch target is "fichero.db.db_manager". find_document_by_path
...
def _family_save_artifacts():
    from fichero.workflows.tools.vision_base import save_artifact as vision_save
    from fichero.workflows.tools.audio_base import save_artifact as audio_save
    from fichero.workflows.tools.video_base import save_artifact as video_save
    from fichero.workflows.tools.extract import save_artifact as extract_save
    return [
        ("vision", vision_save),
        ("audio", audio_save),
        ("video", video_save),
...
def test_all_families_share_one_save_path():
    ...
    """
    from fichero.workflows.tools.llm_base import save_file_artifact
...
def test_family_genuine_miss_returns_none_no_parent_reroute(label, family_save):
    ...
    with patch("fichero.db.db_manager") as mgr, patch(
        "fichero.workflows.tools.llm_base.find_document_by_path",

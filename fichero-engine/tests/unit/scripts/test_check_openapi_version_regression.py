...
partially-installed `fichero-engine/.venv`, so the exported spec carried
...
SCRIPT = ROOT / "fichero-engine" / "scripts" / "check_openapi_version_regression.py"
...
SYNC = ROOT / "fichero-engine" / "scripts" / "sync_openapi_schema.sh"
...
def _fake_tree(tmp_path: Path, exported_version: str) -> tuple[Path, Path]:
    ...
    api_root = tmp_path / "fichero-engine"
    ...
    for name in ("sync_openapi_schema.sh", "check_openapi_version_regression.py"):
        ...
        target.write_text((ROOT / "fichero-engine" / "scripts" / name).read_text())
        target.chmod(0o755)

...
def test_known_vacuous_entry_is_suppressed(monkeypatch):
    vacuous = check_test_assertions.TestEntry("fichero-engine/tests/unit/test_dummy.py::test_known", False)
...
def test_new_vacuous_entry_returns_nonzero(monkeypatch):
    vacuous = check_test_assertions.TestEntry("fichero-engine/tests/unit/test_dummy.py::test_unknown", False)
    monkeypatch.setattr(check_test_assertions, "scan", lambda: [vacuous])

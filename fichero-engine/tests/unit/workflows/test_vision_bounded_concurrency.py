...
These tests patch fichero.llm.vision (process_vision re-imports it per call) and
fichero.workflows.tools.vision_base.file_to_data_uri (so no real image decode),
...
def _fresh_vision_semaphore():
    ...
    import fichero.workflows.builder as _b
...
async def _run(files, vision_fn):
    with (
        ...
        patch("fichero.llm.vision", new=vision_fn),
    ):

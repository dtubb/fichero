...
def test_source_skips_unknown_documents(monkeypatch):
    ...
    monkeypatch.setattr("fichero.db.db_manager.get_database", lambda _path: DB())
...
def test_source_emits_text_crop_with_annotation_metadata(monkeypatch):
    ...
    monkeypatch.setattr("fichero.db.db_manager.get_database", lambda _path: DB())
    monkeypatch.setattr("fichero.workflows.tools._annotation_input.crop_text", lambda _doc, _annotation: "cropped text")

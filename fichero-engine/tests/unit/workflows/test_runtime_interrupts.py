...
def test_create_compiled_app_forwards_interrupts_to_builder(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "fichero.workflows.runtime.AsyncDuckDBCheckpointer.from_db_path",
        lambda db_path: dummy_checkpointer,
    )
    monkeypatch.setattr("fichero.workflows.runtime.build_graph", fake_build_graph)
...
def test_build_graph_forwards_interrupts_to_compile(monkeypatch):
    ...
    monkeypatch.setattr("fichero.workflows.builder.get_tool", fake_get_tool)

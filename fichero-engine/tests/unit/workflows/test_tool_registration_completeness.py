...
import fichero.api.main  # noqa: F401  — breaks the fichero.db <-> api.main import cycle
...
def _tools_imported_by_init() -> set[str]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module == "fichero.workflows.tools":
...
def test_registry_special_cases_no_individual_tools() -> None:
    ...
    offenders = [
        ...
        if "from fichero.workflows.tools import" in line
    ]
...
def test_the_regressed_tools_are_actually_registered() -> None:
    ...
    """
    import fichero.workflows.tools  # noqa: F401  — trigger the decorators

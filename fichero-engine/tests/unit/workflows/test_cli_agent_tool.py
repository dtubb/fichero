...
from fichero.workflows.tools.cli_agent import cli_agent
...
async def test_cli_agent_timeout_kills_process():
    ...
    with patch("fichero.workflows.tools.cli_agent.asyncio.create_subprocess_exec", new=AsyncMock(return_value=proc)):
        with patch("fichero.workflows.tools.cli_agent.asyncio.wait_for", side_effect=_fake_wait_for):
            result = await cli_agent(inputs, state, llm_config)

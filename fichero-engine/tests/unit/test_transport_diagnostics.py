...
from fichero.api.transport_diagnostics import (
    APP_LOOPBACK_URL,
    describe_transport,
...
START_BACKEND_SH = REPO_ROOT / "fichero-engine" / "scripts" / "start_backend.sh"
...
class TestItActuallyGetsLogged:
    def test_log_transport_banner_emits_every_line(self, caplog):
        with caplog.at_level(logging.INFO, logger="fichero.api.transport_diagnostics"):
...
class TestTheScriptsSayItToo:
    ...
    def test_the_python_launcher_prints_it_on_both_paths(self):
        source = (REPO_ROOT / "fichero-engine" / "scripts" / "start_backend.py").read_text()

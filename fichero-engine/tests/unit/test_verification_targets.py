...
from fichero.verification_targets import plan_backend_verification
...
def test_route_change_maps_to_targeted_tests_only() -> None:
    ...
    assert "fichero-engine/tests/unit/test_routes_mcp_tools.py" in plan.pytest_targets
    assert "fichero-engine/tests/unit/test_mcp_tools.py" in plan.pytest_targets
    assert "fichero-engine/tests/unit/test_mcp_knowledge_adapters.py" in plan.pytest_targets
...
def test_god_node_change_requires_full_gate() -> None:
    ...
    assert "fichero-engine/tests/unit/test_db.py" in plan.pytest_targets


def test_contract_change_requires_full_gate_and_includes_changed_test() -> None:
    plan = plan_backend_verification(
        ["fichero-engine/tests/contracts/test_extensibility_guarantee.py"],
        repo_root=ROOT,
    )
...
def test_security_sensitive_path_requires_full_gate() -> None:
    plan = plan_backend_verification(
        ["fichero-engine/src/fichero/api/routes/auth_accounts.py"],
        repo_root=ROOT,
    )

    assert plan.requires_full_gate is True
    assert "security-sensitive backend path changed" in plan.reasons
    assert "fichero-engine/tests/unit/test_auth_accounts.py" in plan.pytest_targets


def test_unmatched_source_is_reported() -> None:
    plan = plan_backend_verification(
        ["fichero-engine/src/fichero/nonexistent_slice.py"],
        repo_root=ROOT,
    )

...
async def test_cancel_not_running():
    ...
    from fichero.api.routes.workflow_execution.threads import cancel_workflow

    with patch(
        "fichero.api.routes.workflow_execution.threads.cancel_workflow.__wrapped__"
        if hasattr(cancel_workflow, "__wrapped__")
        else "fichero.api.routes.workflow_execution.runner._get_workflow_state",
        return_value=None,
    ):
        with patch(
            "fichero.api.routes.workflow_execution.runner._get_workflow_state",
            return_value=None,
        ):
...
def test_cancel_response_defaults():
    from fichero.api.routes.workflow_execution.threads import CancelResponse

...
def test_remote_embeddings_refuse_when_paid_fallbacks_disabled(monkeypatch) -> None:
    ...
    with patch("fichero.llm._paid_remote_fallbacks_enabled", return_value=False):
        with pytest.raises(RuntimeError, match="Paid remote AI fallbacks are disabled"):

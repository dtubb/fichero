...
# Importing ANY route module must import fichero.api.main FIRST (#3950).
#
...
# doing `from fichero.api.main import app`: touching any leaf of fichero.api
# pulled main in eagerly and completely. #3950 had to make that lazy — the
# same eager import was the root of the fichero.db <-> api.main cycle
# (db -> db_manager -> api.change_stream -> api/__init__ -> api.main -> db),
# which made `import fichero.workflows.tools` impossible standalone.
#
# So the ordering moves HERE, where it is actually needed and where it costs
# nothing:
#   - `fichero.api.change_stream` (the db path) lives under fichero.api, NOT
#     under fichero.api.routes, so it no longer drags main in — cycle stays fixed.
#   - `fichero.cli.client` imports route modules directly for their response
...
import fichero.api.main  # noqa: F401,E402  (import-order dependency, see above)

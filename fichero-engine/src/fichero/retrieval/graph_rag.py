...
class GraphAwareRetriever:
    ...
    def _augment_with_kg(
        ...
        *,
        ...
        if not seed_doc_ids or max_kg_claims <= 0:
            return

        seed_claims = self.db.knowledge_claims_for_seed_documents(set(seed_doc_ids))
        if not seed_claims:
            return
        ...
        for _ in range(max(graph_hops, 0)):
            if not frontier:
                break
            new_claims = self.db.knowledge_claims_for_entity_frontier(
                frontier,
                exclude_ids=gathered_claim_ids,
                limit=max(0, max_gathered - len(gathered_claim_ids)),
            )
            if not new_claims:
                break
            for claim in new_claims:
                claim_by_id[claim.id] = claim
            new_claim_ids = {claim.id for claim in new_claims}

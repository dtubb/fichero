...
action's ``execute``, writes an :class:`fichero.models.ActionAudit` row, and
broadcasts a :class:`fichero.api.change_stream.ChangeEvent` to the observable
...
This package exposes the contract that #2014's per-domain workers copy:
    ...
    from fichero.actions import action, registry, ActionContext, ChangeSpec

...
class FicheroClient:
    ...
    # -- mind palace (spatial) ----------------------------------------------
    # Thin wrappers over /api/mind-palace so an MCP agent can read and
    # manipulate a spatial room — place/move/connect/arrange nodes and focus
    # the camera. Returns parsed JSON (envelopes/single objects) directly; the
    # MCP tools surface that to the model. (#1269)
    def palace_rooms(self) -> Any:
        return self.request("GET", "/api/mind-palace/rooms")

    def palace_scene(self, room_id: str) -> Any:
        """Full scene summary for a room (nodes, connections, stacks)."""
        return self.request("GET", f"/api/mind-palace/rooms/{room_id}/scene")

    def palace_place_node(
        self,
        room_id: str,
        node_type: str,
        *,
    ...
    def palace_move_node(
        ...
        *,
        ...
        return self.request(
            "PATCH",
            f"/api/mind-palace/nodes/{node_id}",
            json={
                "position_x": position_x,
                "position_y": position_y,
                "position_z": position_z,
    ...
    def palace_focus(
        self, room_id: str, node_id: str | None = None, *, user_id: str = "user"
    ) -> Any:
        return self.request(
            "POST",

...
Reuses ``fichero.llm``'s quota classification (``ProviderQuotaError`` and
...
time to avoid an import cycle (``fichero.llm`` is heavy and the vision tools
...
def classify_provider_error(exc: BaseException) -> str:
    ...
    Reuses ``fichero.llm``'s quota detection so the two layers stay in
    sync. The import is deferred to avoid an import cycle.
    """
    from fichero.llm import ProviderQuotaError, _is_provider_quota_error

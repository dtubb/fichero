...
*Fichero*, which is Spanish for a **filing cabinet** or a **card index**) is an application to manage documents, to read them, and to process them with AI tools for macOS, iPadOS, and iOS. Its audience are researchers with a collection of images of historical documents, PDFs of books, archival materials, academic articles, hand written fieldwork notes, audio interviews, video recording, transcripts, primary and secondary sources, and maps. It offers a single home for this material, and then a way to build a semantic understanding using AI tools, to allow researchers to read, transcribe, and ask questions of their materials, while always being able to easily find the relevant passage in the source document. Fichero's aim aims to let you build the steps yourself, visually, repeatably, across a whole corpus, to work with a collection. It moves the beyond the chat box as a kind of delphic opaque agent, and lets you see what the machine actually did and how. The aim is that the AI becomes a tool for the researcher, rather than an interlocutor or a plagiarism machine. It aims to surfaces facts and provenance, but it does not do the interpretation for you.

...
- **DELETION SAFETY:**
  ...
  `<worktrees-root>/` AND in `git worktree list`; else stop and surface it. To clear a
  stale bare-sibling, `mv` it aside for the maintainer — don't recursive-delete.
...
```bash
tmux new-window -t fichero-workers -n <name> -c <worktrees-root>/<name> "codex -m gpt-5.4-mini"
sleep 2
...
the same way. **NEVER `pkill -f codex`** — kills the maintainer's own windows; stop a lane with
...
**Ponytail in every brief (standing rule):** the worker applies ponytail —
...
git -C <worktrees-root>/<name> add -A
git -C <worktrees-root>/<name> commit -m "<msg>"   # only if worker left it uncommitted
CX=$(git -C <worktrees-root>/<name> rev-parse HEAD)
...
- **Build-verify** Swift via Xcode MCP `BuildProject` (a full build — incremental
  ...
  ~2–9 GB).
- If green: `git push`, `git worktree remove --force <worktrees-root>/<name>`,
...
## 4. Never run `xcodebuild test` / `verify_all.sh --full` on the maintainer's machine in

...
8. [What replaces the start_fichero_server.sh switches](#8-what-replaces-the-start_fichero_serversh-switches)
...
2. **Launcher switches:** `fichero-server/scripts/start_fichero_server.sh:43-65` reads macOS `defaults` (`fichero.remote_access.enabled`, `fichero.multiuser.enabled`, …); `:114-121` makes `FICHERO_MULTIUSER` **default to 1 only when a public URL is configured**, else 0 — multi-user is coupled to hosting.
...
| 2 | TLS material + SPKI pin | **Yes, fully** | Engine mints and persists cert+pin at launch (`prepare_remote_access_tls`, invoked by both launch paths — `start_fichero_server.sh:88-113`, `EmbeddedBackendService` per critique §3.3); the pin is never typed (field already deleted, `1132b4b47`). Derivation failure → `PairingBlocker.pinNotDerived` + **[Prepare Certificate]** (landed) |
...
## 8. What replaces the start_fichero_server.sh switches

`start_fichero_server.sh` is the **Debug/dev external-engine launcher** (Release embeds and spawns — #3042, `EngineConfig.launchEnvironment` `EngineConfig.swift:573-591`). Today it smuggles product policy in via macOS `defaults`:

| Today (`fichero-server/scripts/start_fichero_server.sh`) | Replacement |
|---|---|
...
| 6 | **Delete the switches:** app drops the multi-user toggle + `FICHERO_MULTIUSER` line (`EngineConfig.swift:178-186,580`); Engine→Backend duplicate surface deleted (#3776 rest, #3777); `start_fichero_server.sh` defaults-coupling removed (§8); the seven `settings_*_tab` flags per #3776 | Low once 5 holds — UI deletion over a proven engine | Build gate; settings panes exist unconditionally |

...
**3. `fichero/fichero/FicheroEngine.entitlements`** — applied when the release DMG re-signs `Fichero Server.app` with Developer ID + hardened runtime. Grants (all hardened-runtime *exception* entitlements, each mapped in the file's comments to a real CPython failure mode):
...
- `launchEmbeddedBackend()` uses **`Foundation.Process`** directly: executable is `Bundle.main.resourcePath + "/Fichero Server.app/Contents/MacOS/Fichero Server"` — a **Briefcase-packaged CPython app nested inside the app's Resources**. Arguments pass TLS cert/key; env passes `FICHERO_PARENT_PID`, `FICHERO_BOOTSTRAP_TOKEN`, `FICHERO_LAUNCH_NONCE`, TLS material, `FICHERO_BIND_HOST`, feature tier.
...
**Engine (`Fichero Server.app`) — the missing piece.** For the MAS build the engine needs a *new, different* entitlements profile containing **exactly**:

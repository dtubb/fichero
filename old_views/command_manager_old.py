...
class CommandManager:
    ...
    def _create_all_commands(self):
        ...
        try:
            ...
            process_folder_cmd = toga.Command(
                ...
                text="Process",
                tooltip="Open a new document window to process files or folders",
                icon=toga.Icon("resources/icons/process/process.png"),
                group=toga.Group.FILE,
                section=0,
                order=3
            )
            ...
            # Plans command (in App menu) - HIDDEN
            # plans_cmd = toga.Command(
            #     self._on_plans,
            #     text="Plans",
            #     tooltip="Open the plans library to manage project plans",
            #     icon=toga.Icon("resources/icons/plan/plan.png"),
            #     group=toga.Group.APP,
            #     section=0,
            #     order=1
            # )
            # self.commands.append(plans_cmd)
            
            # Prompts command (in App menu) - HIDDEN
            # prompts_cmd = toga.Command(
            #     self._on_prompts,
            #     text="Prompts",
            #     tooltip="Open the prompts library to manage LLM prompts",
            #     icon=toga.Icon("resources/icons/prompt/prompt.png"),
            #     group=toga.Group.APP,
            #     section=0,
            #     order=2
            # )
            # self.commands.append(prompts_cmd)
    ...
    def _on_process_folder(self, widget):
        """Handle Process command - create new processing window"""
        try:
            ...
            logger.info("Created new Process window")
            
        except Exception as e:
            logger.error(f"Failed to create Process window: {e}")
            import traceback
            logger.error(traceback.format_exc()) 
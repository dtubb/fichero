...
def log_operation(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        operation_name = f"{func.__module__}.{func.__name__}" if func.__module__ else func.__name__
        ...
        try:
            ...
            if hasattr(logger, 'log_operation'):
                ...
                logger.info("Operation %s completed successfully in %.4fs", operation_name, duration)
            ...
            if hasattr(logger, 'log_operation'):
                logger.log_operation(operation_name, "failure", duration, {"error": str(e)})
            else:
                logger.error("Operation %s failed after %.4fs: %s", operation_name, duration, str(e))
...
def log_api_endpoint(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger = get_logger(func.__module__ or "fichero.api")
        ...
        try:
            ...
            if request:
                ...
                if hasattr(logger, 'log_api_request'):
                    ...
                    logger.info("API %s %s -> %d in %.4fs", method, path, status_code, duration)
            ...
            if request:
                ...
                if hasattr(logger, 'log_api_request'):
                    logger.log_api_request(method, path, status_code, duration, {"error": str(e)})
                else:
                    logger.error("API %s %s -> %d failed in %.4fs: %s", method, path, status_code, duration, str(e))

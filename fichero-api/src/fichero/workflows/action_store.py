...
BUILTIN_ACTIONS = [
    ...
    Action(
        ...
        node_template={
            "type": "llm",
            "data": {"prompt": "Summarize the following text concisely:\n\n{{input}}", "model": "default"}
        },
        is_builtin=True,
        author="Fichero"
    ),
    Action(
        ...
        node_template={
            "type": "llm",
            "data": {"prompt": "Extract the main keywords from this text as comma-separated list:\n\n{{input}}", "model": "default"}
        },
        is_builtin=True,
        author="Fichero"
    ),
    Action(
        ...
        node_template={
            "type": "llm",
            "data": {"prompt": "Translate the following text to {{target_language}}:\n\n{{input}}", "model": "default"}
        },
        is_builtin=True,
        author="Fichero"
    ),
    Action(
        ...
        node_template={
            "type": "llm",
            "data": {"prompt": "Analyze the sentiment of this text:\n\n{{input}}", "model": "default"}
        },
        is_builtin=True,
        author="Fichero"
    ),
...
class ActionStore:
    ...
    def _insert(self, action: Action) -> None:
        ...
        """, [action.id, action.name, action.description, action.category,
              json.dumps(action.tags), action.icon, json.dumps(action.node_template),
              json.dumps(action.nodes), json.dumps(action.edges), action.is_builtin,
              action.is_composite, action.author, action.use_count, action.last_used_at,
    ...
    def _update(self, action: Action) -> None:
        ...
        self.db.duck.execute("""
            ...
            WHERE id=?
        """, [action.name, action.description, action.category, json.dumps(action.tags),
              action.icon, json.dumps(action.node_template), json.dumps(action.nodes),
              json.dumps(action.edges), action.is_composite, action.author,
              action.use_count, action.last_used_at, action.updated_at, action.id])
    ...
    def _row_to_action(self, row: tuple) -> Action:
        ...
        return Action(
            ...
            is_builtin=row[9], is_composite=row[10], author=row[11],
            use_count=row[12], last_used_at=row[13],
            created_at=row[14], updated_at=row[15]
        )

    def get(self, action_id: str) -> Optional[Action]:
        """Get an action by ID."""
        result = self.db.duck.execute("SELECT * FROM actions WHERE id = ?", [action_id]).fetchone()
    ...
    def list_builtin(self) -> list[Action]:
        ...
        rows = self.db.duck.execute("SELECT * FROM actions WHERE is_builtin = TRUE ORDER BY name").fetchall()
        return [self._row_to_action(r) for r in rows]

    def list_custom(self) -> list[Action]:
        """List user-created actions."""
        rows = self.db.duck.execute("SELECT * FROM actions WHERE is_builtin = FALSE ORDER BY name").fetchall()
        return [self._row_to_action(r) for r in rows]

    def list_by_category(self, category: str) -> list[Action]:
        """List actions by category."""
        rows = self.db.duck.execute("SELECT * FROM actions WHERE category = ? ORDER BY name", [category]).fetchall()
        return [self._row_to_action(r) for r in rows]
    ...
    def list_popular(self, limit: int = 10) -> list[Action]:
        ...
        rows = self.db.duck.execute("SELECT * FROM actions ORDER BY use_count DESC LIMIT ?", [limit]).fetchall()
        return [self._row_to_action(r) for r in rows]

    def get_categories(self) -> list[str]:
        """Get all categories."""
        rows = self.db.duck.execute("SELECT DISTINCT category FROM actions ORDER BY category").fetchall()
        return [r[0] for r in rows]

    def search(self, query: Optional[str] = None, category: Optional[str] = None,
               tags: Optional[list[str]] = None) -> list[Action]:
        """Search actions."""
    ...
    def create_from_node(self, name: str, node: dict[str, Any], description: str = "",
                         category: str = "custom", tags: list[str] = None) -> Action:
        """Create action from workflow node."""
        action = Action(name=name, description=description, category=category,
                       tags=tags or [], node_template=node, is_composite=False)
        self.save(action)
        return action

    def create_composite(self, name: str, nodes: list[dict[str, Any]], edges: list[dict[str, Any]],
                        description: str = "", category: str = "custom", tags: list[str] = None) -> Action:
        """Create composite action from multiple nodes."""
        action = Action(name=name, description=description, category=category,
                       tags=tags or [], nodes=nodes, edges=edges, is_composite=True)
        self.save(action)
        return action

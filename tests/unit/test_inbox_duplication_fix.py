...
async def library_manager(temp_library_dir):
    """Create a LibraryManager instance for testing"""
...
class TestInboxDuplicationPrevention:
    ...
    async def test_get_all_collections_creates_inbox_once(self, library_manager):
        ...
        # Count inboxes
        inboxes1 = [c for c in collections1 if c.get('metadata', {}).get('is_inbox')]
        ...
        # Count inboxes again
        inboxes2 = [c for c in collections2 if c.get('metadata', {}).get('is_inbox')]
    ...
    async def test_multiple_get_all_collections_calls(self, library_manager):
        ...
        # Count total inboxes across all results
        total_inboxes_in_results = sum(
            len([c for c in result if c.get('metadata', {}).get('is_inbox')])
            for result in results
        )
    ...
    async def test_inbox_persists_after_manager_restart(self, temp_library_dir):
        ...
        storage1 = LibraryStorage(str(temp_library_dir / "test_library.db"))
        manager1 = LibraryManager(storage1)
        ...
        # Note: LibraryManager doesn't need explicit close(), just create a new instance
        storage2 = LibraryStorage(str(temp_library_dir / "test_library.db"))
        manager2 = LibraryManager(storage2)

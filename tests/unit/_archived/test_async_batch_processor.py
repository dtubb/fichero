...
class TestSelectorEventLoop:
    """Test SelectorEventLoop usage for worker threads on macOS"""

    def test_selector_event_loop_used_in_worker_thread(self):
        """Test that SelectorEventLoop is used instead of CFEventLoop in worker threads"""
        mock_provider = Mock()
        mock_provider.name = "TestProvider"
        mock_provider.process_image_async = AsyncMock(return_value={
            "text": "test",
        ...
        def worker_function():
            # Patch asyncio.SelectorEventLoop to track its usage
            original_selector = asyncio.SelectorEventLoop

            class TrackingSelectorEventLoop(original_selector):
                def __init__(self, *args, **kwargs):
                    loop_type_holder.append('SelectorEventLoop')
                    super().__init__(*args, **kwargs)

            with patch.object(asyncio, 'SelectorEventLoop', TrackingSelectorEventLoop):
                try:
                    run_async_batch(
                        mock_provider,
                        test_paths,
                    ...
                    pass  # May fail due to mocking, but we just want to check loop type

        worker_thread = threading.Thread(target=worker_function, name="fichero-io_0")
        worker_thread.start()
        worker_thread.join(timeout=5)

        # Should have created a SelectorEventLoop
        assert 'SelectorEventLoop' in loop_type_holder

    def test_no_nested_thread_pool_in_worker(self):
        """Test that worker threads don't create nested ThreadPoolExecutor"""
        mock_provider = Mock()
        mock_provider.name = "TestProvider"
        mock_provider.process_image_async = AsyncMock(return_value={
            "text": "test",
        ...
        test_paths = [Path("/test/image.jpg")]
        nested_executor_created = []

        original_executor = __import__('concurrent.futures').futures.ThreadPoolExecutor

        class TrackingExecutor(original_executor):
            def __init__(self, *args, **kwargs):
                nested_executor_created.append(True)
                super().__init__(*args, **kwargs)

        def worker_function():
            # In worker thread, should NOT create another ThreadPoolExecutor
        ...
        nested_executor_created.clear()

        worker_thread = threading.Thread(target=worker_function, name="fichero-io_0")
        worker_thread.start()
        worker_thread.join(timeout=5)

        # Should NOT have created a nested ThreadPoolExecutor
        # (the fix removes the nested executor for worker threads)
        assert len(nested_executor_created) == 0


class TestNestAsyncioIntegration:
    """Test nest_asyncio integration for running event loops"""

    def test_nest_asyncio_available_check(self):
        """Test NEST_ASYNCIO_AVAILABLE constant exists"""
        from fichero.tools.transcribe_providers.async_batch_processor import NEST_ASYNCIO_AVAILABLE

        assert isinstance(NEST_ASYNCIO_AVAILABLE, bool)

    @pytest.mark.asyncio
    async def test_running_loop_uses_nest_asyncio_or_errors(self):
        """Test that running loop scenario uses nest_asyncio or raises clear error"""
        from fichero.tools.transcribe_providers.async_batch_processor import (
            NEST_ASYNCIO_AVAILABLE,
            run_async_batch
        )
        ...
        # We're in a running event loop (pytest-asyncio)
        loop = asyncio.get_running_loop()
        assert loop.is_running()

        if NEST_ASYNCIO_AVAILABLE:
            # Should work with nest_asyncio
            result = await asyncio.to_thread(
                run_async_batch,
                mock_provider,
                test_paths,
            ...
            # Note: to_thread runs in a new thread, so it gets its own loop
            assert len(result) == 1
        else:
            # Without nest_asyncio, should raise RuntimeError
            # (but this test runs in a thread via to_thread, so it won't hit this path)
            pass
